...
var CACHE_TEST =
  /.+\.(js|jpg|gif|webp|svg(#.*)?|ttf|woff|eot|css|png|html|json)$/i;
var config = {};
var updateQueue = [];
var queueTid = null;
let startTime = 0;
...
const logger = {
  ...
  _getPrefix() {
    return (
      "%c" +
      (Date.now() - startTime).toString().padStart(6, "0") +
      " " +
      logPrefix
    );
  },
...
self.addEventListener("fetch", function (ev) {
  if (
    ...
    logger.debug(
      "ignore cache for " + ev.request.method + " " + ev.request.url
    );
