...
qx.Class.define("cv.Application", {
  ...
  */
  construct() {
    ...
    qx.bom.PageVisibility.getInstance().addListener("change", () => {
      this.setActive(
        qx.bom.PageVisibility.getInstance().getVisibilityState() === "visible"
      );
    });
  ...
  */
  ...
  members: {
    ...
    async bootstrap(xml) {
      ...
      engine.xml = xml;
      loader.addListenerOnce("finished", () => {
      ...
      if (isCached) {
        ...
        if (!cacheValid) {
          ...
          engine.addListenerOnce("changeReady", () => {
            // create the objects
        ...
        qx.event.message.Bus.subscribe(
          ...
          function () {
            cv.Config.initialPage =
              cv.TemplateEngine.getInstance().getPageIdByPath(startpage) ||
              "id_";
          }
        );
      }
