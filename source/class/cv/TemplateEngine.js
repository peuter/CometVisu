  ...
  */
  ...
  members: {
    ...
    addAddress(address, id) {
      this.warn(
        "addAddress is deprecated! Please use cv.data.Model.getInstance().addAddress() instead"
      );
    ...
    getAddresses() {
      this.warn(
        "getAddresses is deprecated! Please use cv.data.Model.getInstance().getAddresses() instead"
      );
    ...
    parseXML(loaded_xml, done) {
      ...
      if (pagesNode.getAttribute("backend-openhab-url") !== null) {
        settings.backendOpenHABUrl = pagesNode.getAttribute(
          "backend-openhab-url"
        );
      }
      ...
      if (pagesNode.getAttribute("bind_click_to_widget") !== null) {
        settings.bindClickToWidget =
          pagesNode.getAttribute("bind_click_to_widget") === "true";
      }
      ...
      if (pagesNode.getAttribute("max_mobile_screen_width") !== null) {
        settings.maxMobileScreenWidth = pagesNode.getAttribute(
          ...
          (ev) => {
            if (ev.getData() === design) {
              this.error(
                'Failed to load "' +
                  design +
                  '" design! Falling back to simplified "pure"'
              );
    ...
    setupPage() {
      ...
      this.visu.login(
        true,
        cv.Config.configSettings.credentials,
        function () {
          ...
          document
            .querySelectorAll('link[href*="mobile.css"]')
    ...
    startScreensaver() {
      if (typeof cv.Config.configSettings.screensave_time === "number") {
        ...
        this.screensave.addListener("interval", () => {
          this.scrollToPage();
        });
     ...
     */
    createPages(page, path, flavour, type) {
