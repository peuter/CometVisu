  ...
  statics: {
    ...
    setWidgetLayout(element, path) {
      ...
      elementData.colspanM = parseFloat(
        elementData.colspanM ||
          cv.parser.WidgetParser.lookupM[Math.floor(elementData.colspan)] ||
          elementData.colspan
      );

      elementData.colspanS = parseFloat(
        elementData.colspanS ||
          cv.parser.WidgetParser.lookupS[Math.floor(elementData.colspan)] ||
          elementData.colspan
      );

      if (rowspan) {
        elementData.rowspanClass = cv.ui.layout.Manager.rowspanClass(
    ...
    parseAddress(xml, path, makeAddressListFn) {
      if (xml.nodeName.toLowerCase() !== "page") {
        const data = this.model.getWidgetData(path);
        data.address = cv.parser.WidgetParser.makeAddressList(
          xml,
          path,
          makeAddressListFn
        );
    ...
    parseChildren(xml, path, flavour, pageType) {
      ...
      childs.forEach(function (child, idx) {
        const childData = cv.parser.WidgetParser.parse(
          child,
          path + "_" + idx,
          flavour,
          pageType
