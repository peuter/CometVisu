...
qx.Class.define("cv.ui.PagePartsHandler", {
  ...
  members: {
    ...
    updateTopNavigation(path) {
      ...
      nav.setAttribute(
        "href",
        "javascript:cv.TemplateEngine.getInstance().scrollToPage('" + id + "')"
      );
      ...
      for (let i = 1; i < path.length; i++) {
        ...
        if (pageElem && pageElem.classList.contains("page")) {
          ...
          nav.setAttribute(
            "href",
            "javascript:cv.TemplateEngine.getInstance().scrollToPage('" +
              id +
              "')"
          );
    ...
    fadeNavbar(position, direction, speed) {
      speed =
        speed !== undefined
          ? speed
          : cv.TemplateEngine.getInstance().main_scroll.getSpeed();
      const initCss = {};
      const targetCss = {};
