  ...
  members: {
    ...
    _createChildControlImpl(id, hash) {
      ...
      switch (id) {
        ...
        case "toggle-expert":
          ...
          control.addListener("changeValue", (ev) => {
    ...
    __checkProvider(id, formData, element) {
      ...
      if (provider) {
        ...
        formData.options = [];
        rootNode
          .querySelectorAll("meta > " + type + "s > " + type)
          .forEach((element) => {
    ...
    __loadContent(value, errors) {
      const tree = this.getChildControl("tree");
      const file = this.getFile();
      if (file) {
        const document = qx.xml.Document.fromString(value);
        const schemaElement = this._schema.getElementNode(
