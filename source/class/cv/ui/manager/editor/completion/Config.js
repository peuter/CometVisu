    ...
    getProvider() {
      return {
        ...
        provideCompletionItems: function (model, position) {
          ...
          const textMeta =
            metaEndPos > 0
              ? completeText.substring(0, metaEndPos)
              : completeText;
          const mappingNames = [];
          const stylingNames = [];
          const templates = {};
          ...
          if (templatesStart >= 0) {
            const templatesString = textMeta
              .substring(
                templatesStart + 11,
                textMeta.indexOf("</templates>") - 12
              )
              .replace(/(?:\r\n|\r|\n)/g, "");
            templatesString
              .split("</template>")
              .forEach(function (rawTemplate) {
          ...
          const isContentSearch =
            lastOpenedTag && lastOpenedTag.isContentSearch;
          ...
          if (lastOpenedTag) {
            ...
            while (
              lastChild &&
              lastChild.tagName.toLowerCase() !== "parsererror"
            ) {
              ...
              templateNames.forEach(function (name) {
                templates[name].forEach(function (variableName) {
                  res.push({
                    ...
                    detail: qx.locale.Manager.tr(
                      "Variable from template %1",
                      name
                    ),
          ...
          const currentItem = this.findElements(
            this._schema.getElementNode("pages"),
            searchedElement,
            openedTags.length,
            openedTags.includes("meta")
          );
