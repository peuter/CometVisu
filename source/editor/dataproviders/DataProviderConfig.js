...
var cvProvider =
  parent && parent.cv
    ? parent.cv.ui.manager.editor.data.Provider.getInstance()
    : null;

if (!window.client) {
  window.client =
    parent && parent.cv ? parent.cv.TemplateEngine.getClient() : null;
}
...
var DataProviderConfig = {
  address: {
    _nodeValue: {
      url:
        window.client && window.client.hasProvider("addresses")
          ? window.client.getProviderUrl("addresses")
          : cvProvider
          ? baseRestPath + "/data/addresses"
          : "editor/dataproviders/list_all_addresses.php",
      convert:
        window.client && window.client.hasProvider("addresses")
          ? window.client.getProviderConvertFunction("addresses")
          : null,
      cache: true,
  ...
  rrd: {
    _nodeValue: {
      url:
        window.client && window.client.hasProvider("rrd")
          ? window.client.getProviderUrl("rrd")
          : cvProvider
          ? baseRestPath + "/data/rrds"
          : "editor/dataproviders/list_all_rrds.php",
      convert:
        window.client && window.client.hasProvider("rrd")
          ? window.client.getProviderConvertFunction("rrd")
          : null,
      cache: true,
      userInputAllowed: true,
    },
  ...
  pages: {
    design: {
      url: cvProvider
        ? baseRestPath + "/data/designs"
        : "resource/designs/get_designs.php",
      map(element) {
  ...
  "*": {
    ...
    styling: {
      live() {
        ...
        $("#config")
          .find(".element > .name.nodeType_styling .nameValue")
          .each(function () {
            var text = $(this).text();
    ...
    mapping: {
      live() {
        ...
        $("#config")
          .find(".element > .name.nodeType_mapping .nameValue")
          .each(function () {
            var text = $(this).text();
...
function getInfluxMeasurements(element) {
  var auth = element.attributes.authentication;

  if (!(auth in influxCache)) influxCache[auth] = {};
  ...
  let uri = cvProvider
    ? baseRestPath + "/data/influxdbs"
    : "editor/dataproviders/list_all_influxdbs.php";
  $.ajax(uri + (auth ? "?auth=" + auth : ""), {
    dataType: "json",
    async: true,
    success(result) {
      influxCache[auth][""] = result;
      influxCache[auth]["#callback"].forEach(function (callback) {
        callback(result);
      });
      influxCache[auth]["#callback"].length = 0; // clear array
    },
    error(jqXHR, textStatus, errorThrown) {
      var result = new Result(false, Messages.dataProvider.loadingError, [
        textStatus,
        errorThrown,
      ]);
...
function retrieveInfluxCache(measurement, type, auth) {
  ...
  $.ajax(uri + measurement + (auth ? "&auth=" + auth : ""), {
    ...
    success(result) {
      ...
      influxCache[auth][measurement][type + "#callback"].forEach(function (
        callback
      ) {
        callback(result);
      });
...
function getInfluxTags(element) {
  ...
  while ("influx" !== influx.name) {
    ...
    if (undefined === influx) return []; // this safety measure can not happen without a bug somewhere!
  }
  ...
  var data = retrieveInfluxCache(
    influx.attributes.measurement,
    "tags",
    influx.attributes.authentication
  );
...
function getInfluxFields(element) {
  ...
  var data = retrieveInfluxCache(
    influx.attributes.measurement,
    "fields",
    element.attributes.authentication
  );
