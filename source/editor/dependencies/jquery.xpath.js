...
(function () {
  var cString = window.String,
    ...
    fArray_indexOf = (function () {
      return cArray.prototype.indexOf
        ? function (aValue, vItem) {
            return aValue.indexOf(vItem);
          }
        : function (aValue, vItem) {
            for (
              var nIndex = 0, nLength = aValue.length;
              nIndex < nLength;
              nIndex++
            )
  ...
  function cLexer(sValue) {
    var aMatch = sValue.match(
      /\$?(?:(?![0-9-])(?:[\w-]+|\*):)?(?![0-9-])(?:[\w-]+|\*)|\(:|:\)|\/\/|\.\.|::|\d+(?:\.\d*)?(?:[eE][+-]?\d+)?|\.\d+(?:[eE][+-]?\d+)?|"[^"]*(?:""[^"]*)*"|'[^']*(?:''[^']*)*'|<<|>>|[!<>]=|(?![0-9-])[\w-]+:\*|\s+|./g
    );
    ...
    if (aMatch) {
      ...
      for (var nIndex = 0, nLength = aMatch.length; nIndex < nLength; nIndex++)
        ...
        else if (aMatch[nIndex] == ":)" && nStack) nStack--;
  ...
  cDOMAdapter.prototype.getElementsByTagNameNS = function (
    oNode,
    sNameSpaceURI,
    sLocalName
  ) {
  ...
  function cDynamicContext(oStaticContext, vItem, oScope, oDOMAdapter) {
    ...
    this.dateTime = new cXSDateTime(
      oDate.getFullYear(),
      oDate.getMonth() + 1,
      oDate.getDate(),
      oDate.getHours(),
      oDate.getMinutes(),
      oDate.getSeconds() + oDate.getMilliseconds() / 1000,
      -nOffset
    );

    this.timezone = new cXSDayTimeDuration(
      0,
      cMath.abs(~~(nOffset / 60)),
      cMath.abs(nOffset % 60),
      0,
      nOffset > 0
    );
  ...
  cStaticContext.prototype.defaultCollationName =
    sNS_XPF + "/collation/codepoint";
  ...
  cStaticContext.prototype.getDataType = function (sUri) {
    ...
    if (aMatch)
      return aMatch[1] == sNS_XSD
        ? hStaticContext_dataTypes[cRegExp.$2]
        : this.dataTypes[sUri];
  };
  ...
  cStaticContext.prototype.getFunction = function (sUri) {
    ...
    if (aMatch)
      return aMatch[1] == sNS_XPF
        ? hStaticContext_functions[cRegExp.$2]
        : this.functions[sUri];
  };
  ...
  cStaticContext.prototype.getURIForPrefix = function (sPrefix) {
    var oResolver = this.namespaceResolver,
      fResolver =
        oResolver && oResolver.lookupNamespaceURI
          ? oResolver.lookupNamespaceURI
          : oResolver,
      sNameSpaceURI;
    if (
      fResolver instanceof cFunction &&
      (sNameSpaceURI = fResolver.call(oResolver, sPrefix))
    )
  ...
  cStaticContext.js2xs = function (vItem) {
    if (typeof vItem == "boolean") vItem = new cXSBoolean(vItem);
    else if (typeof vItem == "number")
      vItem =
        fIsNaN(vItem) || !fIsFinite(vItem)
          ? new cXSDouble(vItem)
          : fNumericLiteral_parseValue(cString(vItem));
    else vItem = new cXSString(cString(vItem));
    return vItem;
  };

  cStaticContext.xs2js = function (vItem) {
    if (vItem instanceof cXSBoolean) vItem = vItem.valueOf();
    else if (fXSAnyAtomicType_isNumeric(vItem)) vItem = vItem.valueOf();
    else vItem = vItem.toString();
    return vItem;
  };
  ...
  cExpr.prototype.evaluate = function (oContext) {
    var oSequence = [];
    for (
      var nIndex = 0, nLength = this.items.length;
      nIndex < nLength;
      nIndex++
    )
      oSequence = hStaticContext_operators["concatenate"].call(
        oContext,
        oSequence,
  ...
  cForExpr.prototype.evaluate = function (oContext) {
    ...
    (function (oSelf, nBinding) {
      var oBinding = oSelf.bindings[nBinding++],
        ...
        sUri =
          (oBinding.namespaceURI ? "{" + oBinding.namespaceURI + "}" : "") +
          oBinding.localName;
      for (
        var nIndex = 0, nLength = oSequence1.length;
        nIndex < nLength;
        nIndex++
      ) {
        oContext.pushVariable(sUri, oSequence1[nIndex]);
        if (nBinding < oSelf.bindings.length) arguments.callee(oSelf, nBinding);
        else oSequence = oSequence.concat(oSelf.returnExpr.evaluate(oContext));
  ...
  function fIfExpr_parse(oLexer, oStaticContext) {
    ...
    if (oLexer.peek() == "if" && oLexer.peek(1) == "(") {
      ...
      if (
        oLexer.eof() ||
        !(oThenExpr = fExprSingle_parse(oLexer, oStaticContext))
      )
        throw new cException(
          "XPST0003",
      ...
      if (
        oLexer.eof() ||
        !(oElseExpr = fExprSingle_parse(oLexer, oStaticContext))
      )
        throw new cException(
          "XPST0003",
  ...
  cIfExpr.prototype.evaluate = function (oContext) {
    return this[
      fFunction_sequence_toEBV(this.condExpr.evaluate(oContext), oContext)
        ? "thenExpr"
        : "elseExpr"
    ].evaluate(oContext);
  };
  ...
  function fQuantifiedExpr_parse(oLexer, oStaticContext) {
    ...
    if (
      (sQuantifier == "some" || sQuantifier == "every") &&
      ...
      var oQuantifiedExpr = new cQuantifiedExpr(sQuantifier),
        oExpr;
      do {
        oQuantifiedExpr.bindings.push(
          fSimpleQuantifiedBinding_parse(oLexer, oStaticContext)
        );
      } while (oLexer.peek() == "," && oLexer.next());
  ...
  cQuantifiedExpr.prototype.evaluate = function (oContext) {
    ...
    (function (oSelf, nBinding) {
      ...
      for (
        var nIndex = 0, nLength = oSequence1.length;
        nIndex < nLength && (bEvery ? bResult : !bResult);
        nIndex++
      ) {
        oContext.pushVariable(sUri, oSequence1[nIndex]);
        if (nBinding < oSelf.bindings.length) arguments.callee(oSelf, nBinding);
        else
          bResult = fFunction_sequence_toEBV(
            oSelf.satisfiesExpr.evaluate(oContext),
            oContext
          );
  ...
  function cSimpleQuantifiedBinding(
    sPrefix,
    sLocalName,
    sNameSpaceURI,
    oInExpr
  ...
  function fSimpleQuantifiedBinding_parse(oLexer, oStaticContext) {
    ...
    return new cSimpleQuantifiedBinding(
      aMatch[1] || null,
      aMatch[2],
      aMatch[1] ? oStaticContext.getURIForPrefix(aMatch[1]) : null,
      oExpr
  ...
  function fComparisonExpr_GeneralComp(oExpr, oContext) {
    var oLeft = fFunction_sequence_atomize(
      oExpr.left.evaluate(oContext),
      oContext
    );

    if (!oLeft.length) return new cXSBoolean(false);

    var oRight = fFunction_sequence_atomize(
      oExpr.right.evaluate(oContext),
      oContext
    );
    ...
    var bResult = false;
    for (
      var nLeftIndex = 0, nLeftLength = oLeft.length, bLeft, vLeft;
      nLeftIndex < nLeftLength && !bResult;
      nLeftIndex++
    ) {
      for (
        var nRightIndex = 0, nRightLength = oRight.length, bRight, vRight;
        nRightIndex < nRightLength && !bResult;
        nRightIndex++
      ) {
        ...
        bResult = hComparisonExpr_ValueComp_operators[
          hComparisonExpr_GeneralComp_map[oExpr.operator]
        ](vLeft, vRight, oContext).valueOf();
      }
    }
    return new cXSBoolean(bResult);
  }
  ...
  function fComparisonExpr_ValueComp(oExpr, oContext) {
    ...
    fFunctionCall_assertSequenceCardinality(
      oContext,
      oLeft,
      "?",
    ...
    return hComparisonExpr_ValueComp_operators[oExpr.operator](
      vLeft,
      vRight,
      oContext
    );
  ...
  hComparisonExpr_ValueComp_operators["eq"] = function (
    oLeft,
    oRight,
    oContext
  ) {
    ...
    if (fXSAnyAtomicType_isNumeric(oLeft)) {
      ...
      if (oRight instanceof cXSString)
        return hStaticContext_operators["numeric-equal"].call(
          oContext,
          hStaticContext_functions["compare"].call(oContext, oLeft, oRight),
          new cXSInteger(0)
        );
  ...
  hComparisonExpr_ValueComp_operators["ne"] = function (
    oLeft,
    oRight,
    oContext
  ) {
    return new cXSBoolean(
      !hComparisonExpr_ValueComp_operators["eq"](
        oLeft,
        oRight,
        oContext
      ).valueOf()
  ...
  hComparisonExpr_ValueComp_operators["gt"] = function (
    oLeft,
    oRight,
    oContext
  ) {
    ...
    if (fXSAnyAtomicType_isNumeric(oLeft)) {
      ...
      if (oRight instanceof cXSString)
        return hStaticContext_operators["numeric-greater-than"].call(
          oContext,
          hStaticContext_functions["compare"].call(oContext, oLeft, oRight),
          new cXSInteger(0)
        );
  ...
  hComparisonExpr_ValueComp_operators["lt"] = function (
    oLeft,
    oRight,
    oContext
  ) {
    ...
    if (fXSAnyAtomicType_isNumeric(oLeft)) {
      ...
      if (oRight instanceof cXSString)
        return hStaticContext_operators["numeric-less-than"].call(
          oContext,
          hStaticContext_functions["compare"].call(oContext, oLeft, oRight),
          new cXSInteger(0)
        );
  ...
  hComparisonExpr_ValueComp_operators["ge"] = function (
    oLeft,
    oRight,
    oContext
  ) {
    ...
    if (sOperator)
      return new cXSBoolean(
        !hStaticContext_operators[sOperator]
          .call(oContext, oLeft, oRight)
          .valueOf()
      );

    throw new cException("XPTY0004", "Cannot compare values of given types");
  };
  hComparisonExpr_ValueComp_operators["le"] = function (
    oLeft,
    oRight,
    oContext
  ) {
  ...
  function fComparisonExpr_NodeComp(oExpr, oContext) {
    ...
    fFunctionCall_assertSequenceItemType(
      oContext,
      oLeft,
      cXTNode,
      "first operand of '" + oExpr.operator + "'"
    ...
    return hComparisonExpr_NodeComp_operators[oExpr.operator](
      oLeft[0],
      oRight[0],
      oContext
    );
  ...
  hComparisonExpr_NodeComp_operators["is"] = function (
    oLeft,
    oRight,
    oContext
  ) {
    return hStaticContext_operators["is-same-node"].call(
      oContext,
      oLeft,
      oRight
    );
  };
  hComparisonExpr_NodeComp_operators[">>"] = function (
    oLeft,
    oRight,
    oContext
  ) {
  ...
  hComparisonExpr_NodeComp_operators["<<"] = function (
    ...
    return hStaticContext_operators["node-before"].call(
      oContext,
      oLeft,
      oRight
    );
  ...
  hAdditiveExpr_operators["+"] = function (oLeft, oRight, oContext) {
    ...
    if (sOperator)
      return hStaticContext_operators[sOperator].call(
        oContext,
        bReverse ? oRight : oLeft,
  ...
  hAdditiveExpr_operators["-"] = function (oLeft, oRight, oContext) {
    ...
    if (fXSAnyAtomicType_isNumeric(oLeft)) {
      ...
      if (oRight instanceof cXSDate) sOperator = "subtract-dates";
      else if (oRight instanceof cXSYearMonthDuration)
        sOperator = "subtract-yearMonthDuration-from-date";
      else if (oRight instanceof cXSDayTimeDuration)
        sOperator = "subtract-dayTimeDuration-from-date";
    } else if (oLeft instanceof cXSTime) {
      if (oRight instanceof cXSTime) sOperator = "subtract-times";
      else if (oRight instanceof cXSDayTimeDuration)
        sOperator = "subtract-dayTimeDuration-from-time";
    } else if (oLeft instanceof cXSDateTime) {
      if (oRight instanceof cXSDateTime) sOperator = "subtract-dateTimes";
      else if (oRight instanceof cXSYearMonthDuration)
  ...
  function fAdditiveExpr_parse(oLexer, oStaticContext) {
    ...
    if (
      oLexer.eof() ||
      !(oExpr = fMultiplicativeExpr_parse(oLexer, oStaticContext))
    )
      return;
  ...
  cAdditiveExpr.prototype.evaluate = function (oContext) {
    ...
    for (
      var nIndex = 0, nLength = this.items.length, oRight, vRight;
      nIndex < nLength;
      nIndex++
    ) {
      oRight = fFunction_sequence_atomize(
        this.items[nIndex][1].evaluate(oContext),
        oContext
      );
      ...
      vLeft = hAdditiveExpr_operators[this.items[nIndex][0]](
        vLeft,
        vRight,
        oContext
      );
  ...
  hMultiplicativeExpr_operators["*"] = function (oLeft, oRight, oContext) {
    ...
    if (fXSAnyAtomicType_isNumeric(oLeft)) {
      if (fXSAnyAtomicType_isNumeric(oRight)) sOperator = "numeric-multiply";
      else if (oRight instanceof cXSYearMonthDuration) {
  ...
  hMultiplicativeExpr_operators["idiv"] = function (oLeft, oRight, oContext) {
    if (fXSAnyAtomicType_isNumeric(oLeft) && fXSAnyAtomicType_isNumeric(oRight))
      return hStaticContext_operators["numeric-integer-divide"].call(
        oContext,
        oLeft,
        oRight
      );
  ...
  hMultiplicativeExpr_operators["mod"] = function (oLeft, oRight, oContext) {
    if (fXSAnyAtomicType_isNumeric(oLeft) && fXSAnyAtomicType_isNumeric(oRight))
      return hStaticContext_operators["numeric-mod"].call(
        oContext,
        oLeft,
        oRight
      );
  ...
  cMultiplicativeExpr.prototype.evaluate = function (oContext) {
    ...
    for (
      ...
      vRight = oRight[0];
      if (vRight instanceof cXSUntypedAtomic) vRight = cXSDouble.cast(vRight);
      vLeft = hMultiplicativeExpr_operators[this.items[nIndex][0]](
        vLeft,
        vRight,
        oContext
      );
  ...
  hUnaryExpr_operators["-"] = function (oRight, oContext) {
    if (fXSAnyAtomicType_isNumeric(oRight))
      return hStaticContext_operators["numeric-unary-minus"].call(
        oContext,
        oRight
      );
  ...
  hUnaryExpr_operators["+"] = function (oRight, oContext) {
    if (fXSAnyAtomicType_isNumeric(oRight))
      return hStaticContext_operators["numeric-unary-plus"].call(
        oContext,
        oRight
      );
  ...
  cUnaryExpr.prototype.evaluate = function (oContext) {
    var oRight = fFunction_sequence_atomize(
      this.expression.evaluate(oContext),
      oContext
    );
  ...
  cOrExpr.prototype.evaluate = function (oContext) {
    var bValue = fFunction_sequence_toEBV(
      this.left.evaluate(oContext),
      oContext
    );
    ...
    for (
      var nIndex = 0, nLength = this.items.length;
      nIndex < nLength && !bValue;
      nIndex++
    )
      bValue = fFunction_sequence_toEBV(
        this.items[nIndex].evaluate(oContext),
        oContext
      );
  ...
  function fAndExpr_parse(oLexer, oStaticContext) {
    ...
    if (
      oLexer.eof() ||
      !(oExpr = fComparisonExpr_parse(oLexer, oStaticContext))
    )
      return;
  ...
  function fStepExpr_parse(oLexer, oStaticContext) {
    if (!oLexer.eof())
      return (
        fFilterExpr_parse(oLexer, oStaticContext) ||
  ...
  cStepExpr.prototype.applyPredicates = function (oSequence, oContext) {
    ...
    for (
      var nPredicateIndex = 0,
        oSequence1,
        nPredicateLength = this.predicates.length;
      nPredicateIndex < nPredicateLength;
      nPredicateIndex++
    ) {
      oSequence1 = oSequence;
      oSequence = [];
      for (
        var nIndex = 0, oSequence2, nLength = oSequence1.length;
        nIndex < nLength;
        nIndex++
      ) {
  ...
  function fAxisStep_parse(oLexer, oStaticContext) {
    ...
    if (oLexer.peek(1) == "::") {
      ...
      if (oLexer.eof() || !(oExpr = fNodeTest_parse(oLexer, oStaticContext)))
        return;
      oStep = new cAxisStep(
        oExpr instanceof cKindTest && oExpr.name == "attribute"
          ? "attribute"
          : "child",
        oExpr
      );
  ...
  cAxisStep.prototype.evaluate = function (oContext) {
    ...
    switch (this.axis) {
      case "attribute":
        if (nType == 1)
          for (
            var aAttributes = fGetProperty(oItem, "attributes"),
              nIndex = 0,
              nLength = aAttributes.length;
            nIndex < nLength;
            nIndex++
          )
      ...
      case "child":
        for (
          var oNode = fGetProperty(oItem, "firstChild");
          oNode;
      ...
      case "descendant":
        fAxisStep_getChildrenForward(
          fGetProperty(oItem, "firstChild"),
          oSequence,
          fGetProperty
        );
      ...
      case "ancestor":
        for (
          var oNode = nType == 2 ? fGetProperty(oItem, "ownerElement") : oItem;
      ...
      case "parent":
        var oParent =
          nType == 2
            ? fGetProperty(oItem, "ownerElement")
            : fGetProperty(oItem, "parentNode");
      ...
      case "preceding-sibling":
        for (
          var oNode = oItem;
          (oNode = fGetProperty(oNode, "previousSibling"));
    ...
    if (
      oSequence.length &&
      !(this.test instanceof cKindTest && this.test.name == "node")
    ) {
  ...
  function fPathExpr_parse(oLexer, oStaticContext) {
    ...
    if (sSlash == sDoubleSlash || sSlash == sSingleSlash) {
      ...
      if (sSlash == sDoubleSlash)
        oPathExpr.items.push(
          new cAxisStep("descendant-or-self", new cKindTest("node"))
        );
    }
  ...
  cPathExpr.prototype.evaluate = function (oContext) {
    ...
    for (
      var nItemIndex = 0, nItemLength = this.items.length, oSequence1;
      nItemIndex < nItemLength;
      nItemIndex++
    ) {
      oSequence1 = [];
      for (
        var nIndex = 0, nLength = oSequence.length;
        nIndex < nLength;
        nIndex++
      ) {
        ...
        for (
          var nRightIndex = 0,
            oSequence2 = this.items[nItemIndex].evaluate(oContext),
            nRightLength = oSequence2.length;
          nRightIndex < nRightLength;
          nRightIndex++
        )
          if (
            nItemIndex < nItemLength - 1 &&
  ...
  function fNodeTest_parse(oLexer, oStaticContext) {
    if (!oLexer.eof())
      return (
        fKindTest_parse(oLexer, oStaticContext) ||
  ...
  cKindTest.prototype.test = function (oNode, oContext) {
    ...
    if (nType == 2)
      return (
        fGetProperty(oNode, "prefix") != "xmlns" &&
  ...
  cNameTest.prototype.test = function (oNode, oContext) {
    ...
    if (nType == 1 || nType == 2) {
      ...
      if (this.localName == fGetProperty(oNode, "localName"))
        return (
          this.namespaceURI == "*" ||
          (nType == 2 && !this.prefix && !fGetProperty(oNode, "prefix")) ||
          fGetProperty(oNode, "namespaceURI") == this.namespaceURI
        );
    }
    return false;
  ...
  function fLiteral_parse(oLexer, oStaticContext) {
    if (!oLexer.eof())
      return (
        fNumericLiteral_parse(oLexer, oStaticContext) ||
  ...
  function fNumericLiteral_parseValue(sValue) {
    ...
    if (aMatch) {
      ...
      if (aMatch[5]) cType = cXSDouble;
      else if (aMatch[2] || aMatch[3]) cType = cXSDecimal;
  ...
  cVarRef.prototype.evaluate = function (oContext) {
    var sUri =
      (this.namespaceURI ? "{" + this.namespaceURI + "}" : "") + this.localName;
  ...
  function fFunctionCall_parse(oLexer, oStaticContext) {
    ...
    if (aMatch && oLexer.peek(1) == "(") {
      ...
      var oFunctionCallExpr = new cFunctionCall(
          aMatch[1] || null,
          aMatch[2],
          aMatch[1]
            ? oStaticContext.getURIForPrefix(aMatch[1]) || null
      ...
      if (oLexer.peek() != ")") {
        do {
          if (
            oLexer.eof() ||
            !(oExpr = fExprSingle_parse(oLexer, oStaticContext))
          )
  ...
  cFunctionCall.prototype.evaluate = function (oContext) {
    ...
    if (this.namespaceURI == sNS_XPF) {
      ...
      if (
        (fFunction = hStaticContext_dataTypes[this.localName]) &&
        this.localName != "NOTATION" &&
        this.localName != "anyAtomicType"
      ) {
        fFunctionCall_prepare(
          this.localName,
          [[cXSAnyAtomicType]],
  ...
  function fFunctionCall_prepare(
    sName,
    aParameters,
    fFunction,
    aArguments,
    ...
    while (
      nParametersRequired < aParameters.length &&
    ...
    if (nArgumentsLength > nParametersLength)
      throw new cException(
        ...
        "Function " +
          sName +
          "() must have " +
          (nParametersLength ? " no more than " : "") +
          nParametersLength +
          " argument" +
  ...
  function fFunctionCall_assertSequenceItemType(
    oContext,
    oSequence,
    cItemType,
    sSource
    ...
    for (
      var nIndex = 0, nLength = oSequence.length, nNodeType, vItem;
      nIndex < nLength;
      nIndex++
    ) {
      ...
      if (cItemType == cXTNode || cItemType.prototype instanceof cXTNode) {
        ...
        if (cItemType != cXSAnyAtomicType) {
          if (vItem instanceof cXSUntypedAtomic) vItem = cItemType.cast(vItem);
  ...
  function fFunctionCall_assertSequenceCardinality(
    oContext,
    oSequence,
    sCardinality,
    sSource
  ...
  function fIntersectExceptExpr_parse(oLexer, oStaticContext) {
    ...
    if (
      oLexer.eof() ||
      !(oExpr = fInstanceofExpr_parse(oLexer, oStaticContext))
    )
      return;
    ...
    while (
      (sOperator = oLexer.peek()) == "intersect" ||
      sOperator == "except"
    ) {
      oLexer.next();
      if (
  ...
  cIntersectExceptExpr.prototype.evaluate = function (oContext) {
    ...
    for (
      var nIndex = 0, nLength = this.items.length, oItem;
      nIndex < nLength;
      nIndex++
    )
      oSequence = hStaticContext_operators[
        (oItem = this.items[nIndex])[0]
      ].call(oContext, oSequence, oItem[1].evaluate(oContext));
    return oSequence;
  };
  ...
  function fUnionExpr_parse(oLexer, oStaticContext) {
    ...
    if (
      oLexer.eof() ||
      !(oExpr = fIntersectExceptExpr_parse(oLexer, oStaticContext))
    )
      return;
  ...
  cUnionExpr.prototype.evaluate = function (oContext) {
    ...
    for (
      ...
      oSequence = hStaticContext_operators["union"].call(
        oContext,
        oSequence,
        this.items[nIndex].evaluate(oContext)
      );
  ...
  cCastableExpr.prototype.evaluate = function (oContext) {
    ...
    if (oSequence1.length > 1) return [new cXSBoolean(false)];
    else if (!oSequence1.length) return [new cXSBoolean(sOccurence == "?")];
      ...
      if (e.code == "XPST0017")
        throw new cException(
          ...
          "No value is castable to " +
            (oItemType.prefix ? oItemType.prefix + ":" : "") +
            oItemType.localName
        );
  ...
  cCastExpr.prototype.evaluate = function (oContext) {
    ...
    return [
      this.type.itemType.cast(
        fFunction_sequence_atomize(oSequence1, oContext)[0],
        oContext
      ),
    ];
  };
  ...
  cAtomicType.prototype.test = function (vItem, oContext) {
    var sUri =
        ...
        this.localName,
      cType =
        this.namespaceURI == sNS_XSD
          ? hStaticContext_dataTypes[this.localName]
  ...
  function fXSAnyAtomicType_isNumeric(vItem) {
    return (
      vItem instanceof cXSFloat ||
      vItem instanceof cXSDouble ||
      vItem instanceof cXSDecimal
    );
  }
  ...
  cXSBase64Binary.prototype.primitiveKind =
    cXSAnySimpleType.PRIMITIVE_BASE64BINARY;
  ...
  cXSDate.prototype.toString = function () {
    return (
      fXSDateTime_getDateComponent(this) + fXSDateTime_getTZComponent(this)
    );
  };
        ...
        if (nDay - 1 < fXSDate_getDaysForYearMonth(nYear, nMonth))
          return new cXSDate(
            ...
            aMatch[5]
              ? aMatch[5] == "Z"
                ? 0
                : (aMatch[6] == "-" ? -1 : 1) * (aMatch[7] * 60 + aMatch[8] * 1)
              : null,
            aMatch[1] == "-"
          );
  ...
  function fXSDate_getDaysForYearMonth(nYear, nMonth) {
    return nMonth == 2 &&
      (nYear % 400 == 0 || (nYear % 100 != 0 && nYear % 4 == 0))
      ? 29
  ...
  cXSDateTime.prototype.toString = function () {
    return (
      fXSDateTime_getDateComponent(this) +
      "T" +
  ...
  function fXSDateTime_pad(vValue, nLength) {
    ...
    return (
      (sValue.length < nLength + 1
        ? new cArray(nLength + 1 - sValue.length).join("0")
        : "") + sValue
    );
  }
  ...
  function fXSDateTime_getTimeComponent(oDateTime) {
    ...
    return (
      fXSDateTime_pad(oDateTime.hours) +
      ":" +
  ...
  cXSDouble.cast = function (vValue) {
    ...
    if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
      ...
      if (aMatch)
        return new cXSDouble(
          aMatch[7] ? +aMatch[7].replace("INF", "Infinity") : +vValue
        );
  ...
  function fXSDuration_getYearMonthComponent(oDuration) {
    return (
      (oDuration.year ? oDuration.year + "Y" : "") +
  ...
  function fXSDuration_normalize(oDuration) {
    return fXSYearMonthDuration_normalize(
      fXSDayTimeDuration_normalize(oDuration)
    );
  }
      ...
      if (aMatch) {
        ...
        return new cXSGDay(
          ...
          aMatch[2]
            ? aMatch[2] == "Z"
              ? 0
              : (aMatch[3] == "-" ? -1 : 1) * (aMatch[4] * 60 + aMatch[5] * 1)
            : null
        );
      }
      throw new cException("FORG0001");
  ...
  cXSGMonth.prototype.toString = function () {
    return (
      "-" + "-" + fXSDateTime_pad(this.month) + fXSDateTime_getTZComponent(this)
    );
  };
    ...
    if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
      ...
      if (aMatch) {
        ...
        if (nDay - 1 < fXSDate_getDaysForYearMonth(1976, nMonth))
          return new cXSGMonthDay(
            nMonth,
            nDay,
            aMatch[3]
              ? aMatch[3] == "Z"
                ? 0
                : (aMatch[4] == "-" ? -1 : 1) * (aMatch[5] * 60 + aMatch[6] * 1)
              : null
          );
  ...
  cXSHexBinary.cast = function (vValue) {
    ...
    if (vValue instanceof cXSBase64Binary) {
      var sValue = fWindow_atob(vValue.valueOf()),
        aValue = [];
      for (
        var nIndex = 0, nLength = sValue.length, sLetter;
        nIndex < nLength;
        nIndex++
      ) {
  ...
  cXSTime.prototype.toString = function () {
    return (
      fXSDateTime_getTimeComponent(this) + fXSDateTime_getTZComponent(this)
    );
  };
      ...
      if (aMatch) {
        ...
        return new cXSTime(
          ...
          aMatch[8]
            ? aMatch[8] == "Z"
              ? 0
              : (aMatch[9] == "-" ? -1 : 1) * (aMatch[10] * 60 + aMatch[11] * 1)
            : null
        );
      }
      throw new cException("FORG0001");
  ...
  function fXSTime_normalize(oValue) {
    if (oValue.seconds >= 60 || oValue.seconds < 0) {
      oValue.minutes +=
        ~~(oValue.seconds / 60) -
        (oValue.seconds < 0 && oValue.seconds % 60 ? 1 : 0);
      oValue.seconds =
        (oValue.seconds % 60) +
        (oValue.seconds < 0 && oValue.seconds % 60 ? 60 : 0);
    }
    if (oValue.minutes >= 60 || oValue.minutes < 0) {
      oValue.hours +=
        ~~(oValue.minutes / 60) -
        (oValue.minutes < 0 && oValue.minutes % 60 ? 1 : 0);
      oValue.minutes =
        (oValue.minutes % 60) +
        (oValue.minutes < 0 && oValue.minutes % 60 ? 60 : 0);
    }
    if (oValue.hours >= 24 || oValue.hours < 0) {
      if (oValue instanceof cXSDateTime)
        oValue.day +=
          ~~(oValue.hours / 24) -
          (oValue.hours < 0 && oValue.hours % 24 ? 1 : 0);
      oValue.hours =
        (oValue.hours % 24) + (oValue.hours < 0 && oValue.hours % 24 ? 24 : 0);
    }
    return oValue;
  }
  ...
  cXSYearMonthDuration.prototype.builtInKind =
    cXSConstants.XT_YEARMONTHDURATION_DT;
  ...
  fStaticContext_defineSystemDataType(
    "yearMonthDuration",
  ...
  cXSDayTimeDuration.cast = function (vValue) {
    ...
    if (vValue instanceof cXSString || vValue instanceof cXSUntypedAtomic) {
      ...
      if (aMatch)
        return fXSDayTimeDuration_normalize(
          new cXSDayTimeDuration(
            +aMatch[2] || 0,
            +aMatch[3] || 0,
            +aMatch[4] || 0,
            +aMatch[5] || 0,
  ...
  cXSNonPositiveInteger.prototype.builtInKind =
  ...
  cXSNonNegativeInteger.prototype.builtInKind =
    cXSConstants.NONNEGATIVEINTEGER_DT;
  ...
  hStaticContext_operators["yearMonthDuration-less-than"] = function (
    oLeft,
    oRight
  ) {
    return new cXSBoolean(
  ...
  hStaticContext_operators["yearMonthDuration-greater-than"] = function (
    oLeft,
    oRight
  ) {
    return new cXSBoolean(
  ...
  hStaticContext_operators["dayTimeDuration-less-than"] = function (
    oLeft,
    oRight
  ) {
    return new cXSBoolean(
  ...
  hStaticContext_operators["dayTimeDuration-greater-than"] = function (
    oLeft,
    oRight
  ) {
    return new cXSBoolean(
  ...
  hStaticContext_operators["duration-equal"] = function (oLeft, oRight) {
    return new cXSBoolean(
      oLeft.negative == oRight.negative &&
        fOperator_yearMonthDuration_toMonths(oLeft) ==
          fOperator_yearMonthDuration_toMonths(oRight) &&
        fOperator_dayTimeDuration_toSeconds(oLeft) ==
          fOperator_dayTimeDuration_toSeconds(oRight)
  ...
  hStaticContext_operators["gYearMonth-equal"] = function (oLeft, oRight) {
    return fOperator_compareDateTimes(
      new cXSDateTime(
        oLeft.year,
        oLeft.month,
        fXSDate_getDaysForYearMonth(oLeft.year, oLeft.month),
        0,
        0,
        0,
      ...
      new cXSDateTime(
        oRight.year,
        oRight.month,
        fXSDate_getDaysForYearMonth(oRight.year, oRight.month),
        0,
        0,
        0,
  ...
  hStaticContext_operators["gMonth-equal"] = function (oLeft, oRight) {
    return fOperator_compareDateTimes(
      new cXSDateTime(
        1972,
        oLeft.month,
        fXSDate_getDaysForYearMonth(1972, oRight.month),
        0,
        0,
        0,
        oLeft.timezone == null ? this.timezone : oLeft.timezone
  ...
  hStaticContext_operators["add-yearMonthDurations"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_yearMonthDuration_fromMonths(
      fOperator_yearMonthDuration_toMonths(oLeft) +
        fOperator_yearMonthDuration_toMonths(oRight)
    );
  };

  hStaticContext_operators["subtract-yearMonthDurations"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_yearMonthDuration_fromMonths(
  ...
  hStaticContext_operators["multiply-yearMonthDuration"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_yearMonthDuration_fromMonths(
      fOperator_yearMonthDuration_toMonths(oLeft) * oRight
    );
  };

  hStaticContext_operators["divide-yearMonthDuration"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_yearMonthDuration_fromMonths(
  ...
  hStaticContext_operators["add-dayTimeDurations"] = function (oLeft, oRight) {
    return fOperator_dayTimeDuration_fromSeconds(
      fOperator_dayTimeDuration_toSeconds(oLeft) +
        fOperator_dayTimeDuration_toSeconds(oRight)
    );
  };

  hStaticContext_operators["subtract-dayTimeDurations"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_dayTimeDuration_fromSeconds(
  ...
  hStaticContext_operators["multiply-dayTimeDuration"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_dayTimeDuration_fromSeconds(
      fOperator_dayTimeDuration_toSeconds(oLeft) * oRight
    );
  };

  hStaticContext_operators["divide-dayTimeDuration"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_dayTimeDuration_fromSeconds(
  ...
  hStaticContext_operators["subtract-dateTimes"] = function (oLeft, oRight) {
    return fOperator_dayTimeDuration_fromSeconds(
      fOperator_dateTime_toSeconds(oLeft) - fOperator_dateTime_toSeconds(oRight)
    );
  };
  ...
  hStaticContext_operators["subtract-times"] = function (oLeft, oRight) {
    return fOperator_dayTimeDuration_fromSeconds(
      fOperator_time_toSeconds(oLeft) - fOperator_time_toSeconds(oRight)
    );
  };

  hStaticContext_operators["add-yearMonthDuration-to-dateTime"] = function (
    oLeft,
    oRight
  ) {
  ...
  hStaticContext_operators["add-dayTimeDuration-to-dateTime"] = function (
    oLeft,
    oRight
  ) {
  ...
  hStaticContext_operators["subtract-dayTimeDuration-from-dateTime"] =
    function (oLeft, oRight) {
      return fOperator_addDayTimeDuration2DateTime(oLeft, oRight, "-");
    };

  hStaticContext_operators["add-yearMonthDuration-to-date"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_addYearMonthDuration2DateTime(oLeft, oRight, "+");
  ...
  hStaticContext_operators["add-dayTimeDuration-to-date"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_addDayTimeDuration2DateTime(oLeft, oRight, "+");
  ...
  hStaticContext_operators["subtract-yearMonthDuration-from-date"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_addYearMonthDuration2DateTime(oLeft, oRight, "-");
  ...
  hStaticContext_operators["subtract-dayTimeDuration-from-date"] = function (
    oLeft,
    oRight
  ) {
    return fOperator_addDayTimeDuration2DateTime(oLeft, oRight, "-");
  ...
  hStaticContext_operators["add-dayTimeDuration-to-time"] = function (
    oLeft,
    oRight
  ) {
    var oValue = new cXSTime(
      oLeft.hours,
      oLeft.minutes,
      oLeft.seconds,
      oLeft.timezone
  ...
  hStaticContext_operators["subtract-dayTimeDuration-from-time"] = function (
    oLeft,
    oRight
  ) {
    var oValue = new cXSTime(
  ...
  function fOperator_compareDates(oLeft, oRight, sComparator) {
    return fOperator_compareDateTimes(
      cXSDateTime.cast(oLeft),
      cXSDateTime.cast(oRight),
      sComparator
    );
  }
  ...
  function fOperator_addDayTimeDuration2DateTime(oLeft, oRight, sOperator) {
    ...
    if (oLeft instanceof cXSDate) {
      ...
      oValue.seconds =
        oValue.seconds + oRight.seconds * (sOperator == "-" ? -1 : 1);
      oValue.minutes =
        oValue.minutes + oRight.minutes * (sOperator == "-" ? -1 : 1);
  ...
  function fOperator_dayTimeDuration_toSeconds(oDuration) {
    return (
      (((oDuration.day * 24 + oDuration.hours) * 60 + oDuration.minutes) * 60 +
        oDuration.seconds) *
      (oDuration.negative ? -1 : 1)
    );
  }
  ...
  function fOperator_yearMonthDuration_toMonths(oDuration) {
    return (
      (oDuration.year * 12 + oDuration.month) * (oDuration.negative ? -1 : 1)
    );
  }
  ...
  function fOperator_time_toSeconds(oTime) {
    return (
      oTime.seconds +
      (oTime.minutes -
        (oTime.timezone != null ? oTime.timezone % 60 : 0) +
        (oTime.hours - (oTime.timezone != null ? ~~(oTime.timezone / 60) : 0)) *
          60) *
        60
    );
  }
  ...
  function fOperator_dateTime_toSeconds(oValue) {
    var oDate = new cDate(
      (oValue.negative ? -1 : 1) * oValue.year,
      oValue.month,
      oValue.day,
      0,
      0,
  ...
  hStaticContext_operators["node-before"] = function (oLeft, oRight) {
    return new cXSBoolean(
      !!(this.DOMAdapter.compareDocumentPosition(oLeft, oRight) & 4)
    );
  };

  hStaticContext_operators["node-after"] = function (oLeft, oRight) {
    return new cXSBoolean(
      !!(this.DOMAdapter.compareDocumentPosition(oLeft, oRight) & 2)
    );
  };
  ...
  function fFunctionCall_numeric_getPower(oLeft, oRight) {
    if (
      fIsNaN(oLeft) ||
      cMath.abs(oLeft) == nInfinity ||
      fIsNaN(oRight) ||
      cMath.abs(oRight) == nInfinity
    )
      return 0;
    var aLeft = cString(oLeft).match(rNumericLiteral),
      ...
      nPower = cMath.max(
        1,
        (aLeft[2] || aLeft[3] || "").length +
          (aLeft[5] || 0) * (aLeft[4] == "+" ? -1 : 1),
        (aRight[2] || aRight[3] || "").length +
          (aRight[5] || 0) * (aRight[4] == "+" ? -1 : 1)
      );
  ...
  hStaticContext_operators["numeric-add"] = function (oLeft, oRight) {
    ...
    return fOperator_numeric_getResultOfType(
      oLeft,
      oRight,
  ...
  hStaticContext_operators["numeric-multiply"] = function (oLeft, oRight) {
    ...
    return fOperator_numeric_getResultOfType(
      oLeft,
      oRight,
      (nLeft * nPower * (nRight * nPower)) / (nPower * nPower)
    );
  };
  ...
  hStaticContext_operators["numeric-integer-divide"] = function (
    oLeft,
    oRight
  ) {
  ...
  function fOperator_numeric_getResultOfType(oLeft, oRight, nResult) {
    return new (
      oLeft instanceof cXSInteger &&
      oRight instanceof cXSInteger &&
      nResult == cMath.round(nResult)
        ? cXSInteger
        : cXSDecimal
    )(nResult);
  }
  ...
  hStaticContext_operators["QName-equal"] = function (oLeft, oRight) {
    return new cXSBoolean(
      oLeft.localName == oRight.localName &&
        oLeft.namespaceURI == oRight.namespaceURI
    );
  };
  ...
  hStaticContext_operators["union"] = function (oSequence1, oSequence2) {
    var oSequence = [];
    for (
      var nIndex = 0, nLength = oSequence1.length, oItem;
      nIndex < nLength;
      nIndex++
    ) {
    ...
    for (
      var nIndex = 0, nLength = oSequence2.length, oItem;
      nIndex < nLength;
      nIndex++
    ) {
  ...
  hStaticContext_operators["intersect"] = function (oSequence1, oSequence2) {
    var oSequence = [];
    for (
      var nIndex = 0, nLength = oSequence1.length, oItem, bFound;
      nIndex < nLength;
      nIndex++
    ) {
      if (!this.DOMAdapter.isNode((oItem = oSequence1[nIndex])))
      ...
      bFound = false;
      for (
        var nRightIndex = 0, nRightLength = oSequence2.length;
        nRightIndex < nRightLength && !bFound;
        nRightIndex++
      ) {
  ...
  hStaticContext_operators["to"] = function (oLeft, oRight) {
    var oSequence = [];
    for (
      var nIndex = oLeft.valueOf(), nLength = oRight.valueOf();
      nIndex <= nLength;
      nIndex++
    )
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oItem) {
      ...
      return oItem == null
        ? new cXSString("")
        : cXSString.cast(fFunction_sequence_atomize([oItem], this)[0]);
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oNode) {
      ...
      return cXSAnyURI.cast(
        new cXSString(this.DOMAdapter.getProperty(oNode, "baseURI") || "")
      );
    }
  );
    ...
    function (oNode) {
      if (oNode != null) {
        ...
        if (fGetProperty(oNode, "nodeType") == 9)
          return cXSAnyURI.cast(
            new cXSString(fGetProperty(oNode, "documentURI") || "")
          );
      }
      return null;
    }
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (sUri, sBaseUri) {
      ...
      if (arguments.length < 2) {
        ...
        sBaseUri = new cXSString(
          this.DOMAdapter.getProperty(this.item, "baseURI") || ""
        );
      }
      ...
      if (!oUri.authority) {
        ...
        if (oUri.path.charAt(0) != "/") {
          ...
          for (
            var nIndex = 0, nLength = aUriSegments.length;
            nIndex < nLength;
            nIndex++
          ) {
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oDateTime, oDayTimeDuration) {
      return fFunction_dateTime_adjustTimezone(
        oDateTime,
  ...
  function fFunction_dateTime_getComponent(oDateTime, sName) {
    ...
    if (sName == "timezone") {
      ...
      return sName == "seconds"
        ? new cXSDecimal(nValue)
        : new cXSInteger(nValue);
    }
  }
  ...
  function fFunction_dateTime_adjustTimezone(oDateTime, oTimezone) {
    ...
    if (oTimezone == null) oValue.timezone = null;
    else {
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oNode) {
      ...
      return new cXSString(
        this.DOMAdapter.getProperty(oNode, "localName") || ""
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oNode) {
      ...
      return cXSAnyURI.cast(
        new cXSString(this.DOMAdapter.getProperty(oNode, "namespaceURI") || "")
      );
    }
  );
    ...
    function (sLang, oNode) {
      ...
      for (var aAttributes; oNode; oNode = fGetProperty(oNode, "parentNode"))
        if ((aAttributes = fGetProperty(oNode, "attributes")))
          for (
            var nIndex = 0, nLength = aAttributes.length;
            nIndex < nLength;
            nIndex++
          )
            if (fGetProperty(aAttributes[nIndex], "nodeName") == "xml:lang")
              return new cXSBoolean(
                fGetProperty(aAttributes[nIndex], "value")
                  .replace(/-.+/, "")
                  .toLowerCase() ==
                  sLang.valueOf().replace(/-.+/, "").toLowerCase()
              );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oNode) {
      ...
      for (
        var oParent = oNode;
        oParent;
        oParent = fGetProperty(oNode, "parentNode")
      )
        oNode = oParent;
    ...
    function (oValue, oPrecision) {
      ...
      if (nPrecision < 0) {
        var oPower = new cXSInteger(cMath.pow(10, -nPrecision)),
          nRounded = cMath.round(
            hStaticContext_operators["numeric-divide"].call(
              this,
              oValue,
              oPower
            )
        ...
        nDecimal = cMath.abs(
          hStaticContext_operators["numeric-subtract"].call(
            this,
            oRounded,
            hStaticContext_operators["numeric-divide"].call(
              this,
        ...
        return hStaticContext_operators["numeric-multiply"].call(
          this,
          hStaticContext_operators["numeric-add"].call(
            this,
            oRounded,
            new cXSDecimal(nDecimal == 0.5 && nRounded % 2 ? -1 : 0)
          ),
        ...
        var oPower = new cXSInteger(cMath.pow(10, nPrecision)),
          nRounded = cMath.round(
            hStaticContext_operators["numeric-multiply"].call(
              this,
              oValue,
              oPower
            )
        ...
        return hStaticContext_operators["numeric-divide"].call(
          this,
          hStaticContext_operators["numeric-add"].call(
            this,
            oRounded,
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oPrefix, oElement) {
      var sPrefix = oPrefix == null ? "" : oPrefix.valueOf(),
        sNameSpaceURI = this.DOMAdapter.lookupNamespaceURI(
          oElement,
          sPrefix || null
        );

      return sNameSpaceURI == null
        ? null
        : cXSAnyURI.cast(new cXSString(sNameSpaceURI));
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    "index-of",
    [[cXSAnyAtomicType, "*"], [cXSAnyAtomicType], [cXSString, "", true]],
    function (oSequence1, oSearch, oCollation) {
      ...
      var oSequence = [];
      for (
        var nIndex = 0, nLength = oSequence1.length, vRight;
        nIndex < nLength;
        nIndex++
      ) {
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oSequence1, oCollation) {
      ...
      var oSequence = [];
      for (
        var nIndex = 0, nLength = oSequence1.length, vLeft;
        nIndex < nLength;
        nIndex++
      ) {
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oSequence1, oPosition, oSequence3) {
      ...
      else if (nPosition > nLength) nPosition = nLength + 1;
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oSequence1) {
      ...
      try {
        ...
        for (
          var nIndex = 1, nLength = oSequence1.length, vRight;
          nIndex < nLength;
          nIndex++
        ) {
          vRight = oSequence1[nIndex];
        ...
        return hMultiplicativeExpr_operators["div"](
          vValue,
          new cXSInteger(nLength),
          this
        );
      } catch (e) {
        throw e.code != "XPTY0004"
          ? e
          : new cException(
              "FORG0006",
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oSequence1, oCollation) {
      ...
      try {
        var vValue = oSequence1[0];
        for (
          var nIndex = 1, nLength = oSequence1.length;
          nIndex < nLength;
          nIndex++
        )
          if (
            hComparisonExpr_ValueComp_operators["ge"](
              oSequence1[nIndex],
              vValue,
              this
            ).valueOf()
    ...
    function (oSequence1, oCollation) {
      ...
      try {
        ...
        for (
          ...
          if (
            hComparisonExpr_ValueComp_operators["le"](
              oSequence1[nIndex],
              vValue,
              this
            ).valueOf()
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oSequence1, oZero) {
      if (!oSequence1.length) {
        if (arguments.length > 1) return oZero;
        else return new cXSDouble(0);
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oSequence1, oNode) {
      ...
      for (var nIndex = 0; nIndex < oSequence1.length; nIndex++)
        for (
          var nRightIndex = 0,
            aValue = fString_trim(oSequence1[nIndex]).split(/\s+/),
            nRightLength = aValue.length;
          nRightIndex < nRightLength;
          nRightIndex++
        )
          if (
            (oNode = this.DOMAdapter.getElementById(
              oDocument,
              aValue[nRightIndex]
            )) &&
  ...
  function fFunction_sequence_atomize(oSequence1, oContext) {
    var oSequence = [];
    for (
      var nIndex = 0, nLength = oSequence1.length, oItem, vItem;
      nIndex < nLength;
      nIndex++
    ) {
      oItem = oSequence1[nIndex];
      vItem = null;
      if (oItem == null) vItem = null;
        ...
        switch (fGetProperty(oItem, "nodeType")) {
          ...
          case 9:
            ...
            vItem = new cXSUntypedAtomic(
              oNode ? fGetProperty(oNode, "textContent") : ""
            );
  ...
  function fFunction_sequence_order(oSequence1, oContext) {
    return oSequence1.sort(function (oNode, oNode2) {
      var nPosition = oContext.DOMAdapter.compareDocumentPosition(
        oNode,
        oNode2
      );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue1, oValue2, oCollation) {
      ...
      vCollation =
        sCollation == sNS_XPF + "/collation/codepoint"
          ? oCodepointStringCollator
      ...
      return new cXSInteger(
        vCollation.compare(oValue1.valueOf(), oValue2.valueOf())
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction("concat", null, function () {
    ...
    var aValue = [];
    for (
      var nIndex = 0, nLength = arguments.length, oSequence;
      nIndex < nLength;
      nIndex++
    ) {
      ...
      if (oSequence.length)
        aValue[aValue.length] = cXSString
          .cast(fFunction_sequence_atomize(oSequence, this)[0])
          .valueOf();
    }
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      if (!arguments.length) {
        if (!this.item) throw new cException("XPDY0002");
        oValue = cXSString.cast(
          fFunction_sequence_atomize([this.item], this)[0]
        );
      }
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      ...
      return new cXSString(
        oValue == null ? "" : fString_trim(oValue).replace(/\s\s+/g, " ")
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      return new cXSString(
        oValue == null ? "" : oValue.valueOf().toUpperCase()
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      return new cXSString(
        oValue == null ? "" : oValue.valueOf().toLowerCase()
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue, oMap, oTranslate) {
      ...
      var aValue = oValue.valueOf().split(""),
        ...
        aReturn = [];
      for (
        var nIndex = 0, nLength = aValue.length, nPosition;
        nIndex < nLength;
        nIndex++
      )
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      return new cXSString(
        oValue == null ? "" : window.encodeURIComponent(oValue)
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      return new cXSString(
        oValue == null ? "" : window.encodeURI(window.decodeURI(oValue))
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue) {
      ...
      var aValue = oValue.valueOf().split("");
      for (
        var nIndex = 0, nLength = aValue.length, nCode;
        nIndex < nLength;
        nIndex++
      )
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue, oSearch, oCollation) {
      return new cXSBoolean(
        (oValue == null ? "" : oValue.valueOf()).indexOf(
          oSearch == null ? "" : oSearch.valueOf()
        ) >= 0
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue, oSearch, oCollation) {
      ...
      return new cXSBoolean(
        sValue.indexOf(sSearch) == sValue.length - sSearch.length
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue, oSearch, oCollation) {
      ...
      return new cXSString(
        (nPosition = sValue.indexOf(sSearch)) >= 0
          ? sValue.substring(0, nPosition)
          : ""
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue, oSearch, oCollation) {
      ...
      return new cXSString(
        (nPosition = sValue.indexOf(sSearch)) >= 0
          ? sValue.substring(nPosition + sSearch.length)
          : ""
      );
    }
  );
  ...
  fStaticContext_defineSystemFunction(
    "matches",
    [[cXSString, "?"], [cXSString], [cXSString, "", true]],
    function (oValue, oPattern, oFlags) {
      var sValue = oValue == null ? "" : oValue.valueOf(),
        rRegExp = fFunction_string_createRegExp(
          oPattern.valueOf(),
  ...
  fStaticContext_defineSystemFunction(
    "replace",
    [[cXSString, "?"], [cXSString], [cXSString], [cXSString, "", true]],
  ...
  fStaticContext_defineSystemFunction(
    ...
    function (oValue, oPattern, oFlags) {
      ...
      var oSequence = [];
      for (
        var nIndex = 0, aValue = sValue.split(rRegExp), nLength = aValue.length;
        nIndex < nLength;
        nIndex++
      )
    ...
    if (sName == "baseURI") {
      ...
      for (var oParent = oNode, sUri; oParent; oParent = oParent.parentNode)
        if (oParent.nodeType == 1 && (sUri = oParent.getAttribute("xml:base")))
          sBaseURI = fResolveUri(
            new cXSString(sUri),
            new cXSString(sBaseURI)
          ).toString();
      return sBaseURI;
      ...
      (function (oNode) {
        for (
          var nIndex = 0, oChild;
          (oChild = oNode.childNodes[nIndex]);
          nIndex++
        )
  ...
  cLXDOMAdapter.prototype.compareDocumentPosition = function (oNode, oChild) {
    ...
    if (oAttr1 && oAttr2 && oNode && oNode == oChild) {
      for (
        nIndex = 0,
          aAttributes = this.getProperty(oNode, "attributes"),
          nLength = aAttributes.length;
        nIndex < nLength;
        nIndex++
      ) {
    ...
    if (
      (oNode.ownerDocument || oNode) != (oChild.ownerDocument || oChild) ||
      aChain1[aChain1.length - 1] != aChain2[aChain2.length - 1]
    )
      return 32 | 1;
    for (
      nIndex = cMath.min(
        (nLength1 = aChain1.length),
        (nLength2 = aChain2.length)
      );
      nIndex;
      --nIndex
    )
      if ((oNode1 = aChain1[--nLength1]) != (oNode2 = aChain2[--nLength2])) {
        ...
        for (
          oElement = oNode2.previousSibling;
          oElement;
  ...
  cLXDOMAdapter.prototype.getElementsByTagNameNS = function (
    oNode,
    sNameSpaceURI,
    sLocalName
  ) {
    ...
    (function (oNode) {
      for (
        ...
        if (oChild.nodeType == 1) {
          if (
            (bLocalName ||
              sLocalName == this.getProperty(oChild, "localName")) &&
            (bNameSpaceURI ||
              sNameSpaceURI == this.getProperty(oChild, "namespaceURI"))
          )
    ...
    if (sName == "attributes" && oNode.nodeType == 1) {
      ...
      for (
        ...
        if (oNode2.specified) {
          ...
          oAttribute.name =
            oAttribute.nodeName =
            oAttribute.localName =
              oNode2.nodeName.toLowerCase();
  ...
  oXMLStaticContext.defaultFunctionNamespace =
    oHTMLStaticContext.defaultFunctionNamespace;
  ...
  function fXPath_evaluate(oQuery, sExpression, fNSResolver) {
    ...
    var oStaticContext =
      oNode &&
      (oNode.nodeType == 9 ? oNode : oNode.ownerDocument).createElement("div")
        .tagName == "DIV"
        ? oHTMLStaticContext
        : oXMLStaticContext;
    ...
    if (bOldMS)
      oAdapter =
        oStaticContext == oHTMLStaticContext
          ? oMSHTMLDOMAdapter
          : oMSXMLDOMAdapter;
    else if (bOldW3 && oStaticContext == oHTMLStaticContext)
      oAdapter = oL2HTMLDOMAdapter;

    aSequence = oExpression.evaluate(
      new cDynamicContext(oStaticContext, oNode, null, oAdapter)
    );

    for (
      var nIndex = 0, nLength = aSequence.length, oItem;
      nIndex < nLength;
      nIndex++
    )
      oSequence.push(
        oAdapter.isNode((oItem = aSequence[nIndex]))
          ? oItem
  ...
  oObject.xpath = function (oQuery, sExpression, fNSResolver) {
    return fXPath_evaluate(
      oQuery instanceof cQuery ? oQuery : new cQuery(oQuery),
      sExpression,
      fNSResolver
    );
  };
