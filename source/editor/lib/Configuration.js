...
var Configuration = function (filename, isDemo) {
  ...
  _config.load = function () {
    $.ajax(_filename, {
      ...
      error(jqXHR, textStatus, errorThrown) {
        var result = new Result(false, Messages.configuration.loadingError, [
          textStatus,
          errorThrown,
        ]);
  ...
  _config.save = function (filename) {
    if (isDemo) {
      ...
      var result = new Result(false, Messages.configuration.savingError, [
        message,
      ]);
      ...
      window.saveFromIframe(data, filename, function (err) {
        if (err) {
          var result = new Result(
            false,
            Messages.configuration.savingErrorServer,
      ...
      $.ajax("editor/bin/save_config.php", {
        ...
        success(data) {
          if (data === undefined || typeof data.success === "undefined") {
            ...
            var result = new Result(
              false,
              Messages.configuration.savingErrorUnknown
            );
  ...
  _config.setSchema = function (schema) {
    ...
    $.each(_config.rootNodes, function (i, element) {
      ...
      if (typeof _schema.allowedRootElements[elementName] == "undefined") {
        throw (
          "schema is not valid for this configuration, can not find root-level element " +
          elementName
        );
      }
...
var ConfigurationElement = function (node, parent) {
  ...
  _element.setSchemaElement = function (schemaElement) {
    ...
    if (_element.children.length > 0) {
      ...
      $.each(_element.children, function (i, child) {
        ...
        if (
          childSchemaElement === undefined &&
          schemaElement.name === "custom"
        ) {
        ...
        if (childSchemaElement === undefined) {
          ...
          throw (
            "xsd does not match this configuration, or configuration is not valid for " +
            childName
          );
        }
  ...
  _element.isValid = function () {
    ...
    $.each(_element.attributes, function (name, value) {
      ...
      if (typeof _schemaElement.allowedAttributes[name] === "undefined") {
        informListeners("invalid", {
          type: "attribute_disallowed",
          item: name,
        });
      ...
      isValid =
        isValid && _schemaElement.allowedAttributes[name].isValueValid(value);
    });
    ...
    $.each(_schemaElement.allowedAttributes, function (name, attribute) {
      if (false === attribute.isOptional) {
        if (
          typeof _element.attributes[name] === "undefined" ||
          _element.attributes[name] === undefined
        ) {
    ...
    for (var i = 0, len = _element.children.length; i < len; i++) {
      ...
      if (child.name === "#text") {
        // this is a text
        if (
          false === _schemaElement.isMixed &&
          false === _schemaElement.isTextContentAllowed()
        ) {
  ...
  _element.setAttributeValue = function (name, value) {
    ...
    if (
      _schemaElement.allowedAttributes[name].isOptional &&
      value === undefined
    ) {
    ...
    if (false === isValid) {
      ...
      var typeDescription =
        _schemaElement.allowedAttributes[name].getTypeString();
      return new Result(false, Messages.validity.valueInvalidForType, [
        typeDescription,
      ]);
    }
  ...
  _element.remove = function () {
    if (
      typeof _parentElement.isMaster !== "undefined" &&
      _parentElement.isMaster === true
    ) {
  ...
  _element.addChildAtPosition = function (child, position) {
    ...
    for (
      var i = 0;
      i < finalPosition && finalPosition <= _element.children.length;
      i++
    ) {
  ...
  _element.createChildNode = function (childName) {
    ...
    childNode.setSchemaElement(
      _schemaElement.getSchemaElementForElementName(childName)
    );
  ...
  _element.initFromScratch = function () {
    ...
    $.each(
      _schemaElement.allowedAttributes,
      function (attributeName, attributeSchema) {
        if (attributeSchema.defaultValue !== undefined) {
          _element.setAttributeValue(
            attributeName,
