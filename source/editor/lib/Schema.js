...
var xsdNamespaceResolver = function (prefix) {
  if (prefix == "xsd") return "http://www.w3.org/2001/XMLSchema";
...
$.getJSON(
  window.documentationMappingPrefix + "lib/DocumentationMapping.json",
  function (json) {
...
var Schema = function (filename, content) {
  ...
  _schema.getReferencedNode = function (type, refName) {
    if (
      typeof referencedNodeCache[type] != "undefined" &&
      typeof referencedNodeCache[type][refName] != "undefined"
    ) {
  ...
  _schema.getTypeNode = function (type, name) {
    if (
      typeof typeNodeCache[type] != "undefined" &&
      typeof typeNodeCache[type][name] != "undefined"
    ) {
  ...
  _schema.getTextNodeSchemaElement = function () {
    if (textNodeSchemaElement == undefined) {
      ...
      var tmpXML = $("<element />", _schema.getSchemaDOM()).attr({
        name: "#text",
        type: "xsd:string",
      });
...
var SchemaSimpleType = function (node, schema) {
  ...
  var fillNodeData = function (node) {
    ...
    if (
      $n.is(
        fixNamespace(
          "xsd\\:attribute[type], xsd\\:element[type], [name=#text][type]"
        )
      )
    ) {
    ...
    var subNodes = $n.find(
      fixNamespace(
        "> xsd\\:restriction, > xsd\\:extension, > xsd\\:simpleType > xsd\\:restriction, > xsd\\:simpleType > xsd\\:extension"
      )
    );
...
var SchemaAttribute = function (node, schema) {
  ...
  _attribute.getAppinfo = function () {
    ...
    $.each(
      $node.xpath("xsd:annotation/xsd:appinfo", xsdNamespaceResolver),
      function (i, appinfoNode) {
    ...
    if ($node.is("[ref]")) {
      ...
      $.each(
        $ref.xpath("xsd:annotation/xsd:appinfo", xsdNamespaceResolver),
        function (i, appinfoNode) {
          var appinfoNodeText = $(appinfoNode).text();
          appinfo.push(appinfoNodeText);
        }
    ...
    return text.replace(
      ...
      function (match, contents) {
        ...
        return (
          ...
          documentationMapping._base +
          language +
          documentationMapping[key] +
          '">' +
          label +
          "</a>"
        );
  ...
  _attribute.getDocumentation = function () {
    ...
    // any appinfo this element itself might carry
    $.each(
      $node.xpath(selector, xsdNamespaceResolver),
      function (i, documentationNode) {
    ...
    if ($node.is("[ref]")) {
      ...
      $.each(
        $ref.xpath(selector, xsdNamespaceResolver),
        function (i, documentationNode) {
          var documentationNodeText = $(documentationNode).text();
          documentation.push(
            createDocumentationWebLinks(documentationNodeText)
          );
        }
      );
    }
...
var SchemaElement = function (node, schema) {
  ...
  _element.getAllowedContent = function () {
    ...
    if ($type.children(fixNamespace("xsd\\:simpleContent")).length > 0) {
      ...
      allowedContent._text = new SchemaSimpleType(
        $type.children(fixNamespace("xsd\\:simpleContent")),
        _schema
      );
    } else if (
      $type.children(fixNamespace("xsd\\:choice, xsd\\:sequence, xsd\\:group"))
        .length > 0
    ) {
      // we have a choice, group or sequence. great
      // as per the W3C, only one of these may appear per element/type

      var tmpDOMGrouping = $type
        .children(fixNamespace("xsd\\:choice, xsd\\:sequence, xsd\\:group"))
        .get(0);
      ...
      switch (tmpDOMGrouping.nodeName) {
        ...
        case "xsd:sequence":
        case "sequence":
          allowedContent._grouping = new SchemaSequence(
            tmpDOMGrouping,
            _schema
          );
  ...
  var getAllowedAttributes = function () {
    ...
    // allowed attributes
    var attributes = $type.find(
      fixNamespace(
        "> xsd\\:attribute, > xsd\\:simpleContent > xsd\\:extension > xsd\\:attribute"
      )
    );

    // now add any attribute that comes from an attribute-group
    var attributeGroups = $type.find(
      fixNamespace(
        "> xsd\\:attributeGroup, > xsd\\:simpleContent > xsd\\:extension > xsd\\:attributeGroup"
      )
    );
    ...
    attributeGroups.each(function () {
      ...
      if ($(this).is("[ref]")) {
        ...
        $attributeGroup = _schema.getReferencedNode(
          "attributeGroup",
          $(this).attr("ref")
        );
      } else {
      ...
      $attributeGroup
        .children(fixNamespace("xsd\\:attribute"))
        .each(function () {
  ...
  _element.getAppinfo = function () {
    ...
    // any appinfo this element itself might carry
    $.each(
      $e.xpath("xsd:annotation/xsd:appinfo", xsdNamespaceResolver),
      function (i, appinfoNode) {
        var appinfoNodeText = $(appinfoNode).text();
        appinfo.push(appinfoNodeText);
      }
    ...
    if ($e.find(fixNamespace("> xsd\\:complexType")).length == 0) {
      $.each(
        $type.xpath("xsd:annotation/xsd:appinfo", xsdNamespaceResolver),
        function (i, appinfoNode) {
          var appinfoNodeText = $(appinfoNode).text();
          appinfo.push(appinfoNodeText);
        }
  ...
  _element.getDocumentation = function () {
    ...
    // any appinfo this element itself might carry
    $.each(
      $e.xpath(selector, xsdNamespaceResolver),
      function (i, documentationNode) {
        var documentationNodeText = $(documentationNode).text();
    ...
    if ($e.find(fixNamespace("> xsd\\:complexType")).length == 0) {
      $.each(
        $type.xpath(selector, xsdNamespaceResolver),
        function (i, documentationNode) {
          var documentationNodeText = $(documentationNode).text();
          documentation.push(documentationNodeText);
        }
...
var SchemaGroup = function (node, schema) {
  ...
  var parse = function () {
    ...
    // sequences
    $.each(
      $group.find(fixNamespace("> xsd\\:sequence")),
      function (i, grouping) {
        subGroupings.push(new SchemaSequence(grouping, _schema));
      }
    );
...
var SchemaSequence = function (node, schema) {
  ...
  _sequence.getBoundsForElementName = function (childName) {
    ...
    if (typeof allowedElements[childName] !== "undefined") {
      ...
      if (
        elementBounds.min == "unbounded" ||
        sequenceBounds.min == "unbounded"
      ) {
      ...
      if (
        elementBounds.max == "unbounded" ||
        sequenceBounds.max == "unbounded"
      ) {
