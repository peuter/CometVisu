...
function getOffsetCorners(elem) {
  ...
  return {
    ...
    bottom_left: {
      top: Math.round(bounds.top + bounds.height),
      left: Math.round(bounds.left),
    },
    top_right: {
      top: Math.round(bounds.top),
      left: Math.round(bounds.left + bounds.width),
    },
    bottom_right: {
      top: Math.round(bounds.top + bounds.height),
      left: Math.round(bounds.left + bounds.width),
...
function roundCorners() {
  ...
  document
    .querySelectorAll(".page.activePage .group")
    .forEach(function (group) {
      ...
      if (
        Array.prototype.filter.call(
          qx.dom.Hierarchy.getAncestors(group),
          function (m) {
      ...
      var bottomRight =
        window.getComputedStyle(group)["border-bottom-right-radius"];
      var bottomLeft =
        window.getComputedStyle(group)["border-bottom-left-radius"];
      var roundUpperCorners =
        group.querySelectorAll(".widget_container:first-child").length > 0 &&
        topRight !== "0px";
      var threshold = 5;
      group.querySelectorAll(".widget_container").forEach(function (elem) {
        ...
        if (roundUpperCorners) {
          ...
          if (
            Math.abs(elemCorners.top_right.top - groupCorners.top_right.top) <
              threshold &&
            Math.abs(elemCorners.top_right.left - groupCorners.top_right.left) <
              threshold
          ) {
        ...
        if (
          bottomRight !== "0px" &&
          Math.abs(
            elemCorners.bottom_right.top - groupCorners.bottom_right.top
          ) < threshold &&
          Math.abs(
            elemCorners.bottom_right.left - groupCorners.bottom_right.left
          ) < threshold
        ) {
        ...
        if (
          bottomLeft !== "0px" &&
          Math.abs(elemCorners.bottom_left.top - groupCorners.bottom_left.top) <
            threshold &&
          Math.abs(
            elemCorners.bottom_left.left - groupCorners.bottom_left.left
          ) < threshold
        ) {
...
qx.event.message.Bus.subscribe("setup.dom.finished", function () {
  document
    .querySelectorAll(
      "#navbarLeft .navbar .widget .label,#navbarRight .navbar .widget .label"
    )
    .forEach(function (label) {
      if (label.textContent.trim() !== "") {
        var actor = Array.prototype.filter.call(
          qx.dom.Hierarchy.getSiblings(label),
          function (m) {
        ...
        if (
          actor &&
          Array.from(label.children).filter(function (m) {
            return m.matches("img");
          }).length === 0
        ) {
  ...
  document
    .querySelectorAll(".page > div > .widget_container > .group:not(.widget)")
    .forEach(function (elem) {
      if (
        elem.querySelectorAll(".clearfix > .widget_container > .group.widget")
          .length > 0
      ) {
        Object.entries({ border: "none", margin: 0 }).forEach(function (
          key_value
        ) {
          elem.style[key_value[0]] = key_value[1];
        });
      }
    });
});
