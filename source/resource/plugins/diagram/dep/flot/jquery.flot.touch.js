  ...
  function init(plot) {
    ...
    function pan(delta) {
      ...
      switch (options.touch.pan.toLowerCase()) {
        case "x":
          placeholder.css(
            "transform",
            "translateX(" + relativeOffset.x + "px)"
          );
          break;
        case "y":
          placeholder.css(
            "transform",
            "translateY(" + relativeOffset.y + "px)"
          );
          break;
        default:
          placeholder.css(
            "transform",
            "translate(" + relativeOffset.x + "px," + relativeOffset.y + "px)"
          );
          break;
      }
    }
    ...
    function bindEvents(plot, eventHolder) {
      ...
      placeholder.bind("touchstart", function (evt) {
        ...
        $.each(plot.getAxes(), function (index, axis) {
          if (
            axis.direction === options.touch.scale.toLowerCase() ||
            options.touch.scale.toLowerCase() == "xy"
          ) {
            axis.touch = {
              min: axis.min,
              max: axis.max,
      ...
      placeholder.bind("touchend", function (evt) {
        ...
        if (
          isPanning &&
          touches.length === 1 &&
          tapTimestamp + options.touch.tapThreshold - getTimestamp() >= 0 &&
          startTouchPosition.x >=
            lastTouchPosition.x - options.touch.tapPrecision &&
          startTouchPosition.x <=
            lastTouchPosition.x + options.touch.tapPrecision &&
          startTouchPosition.y >=
            lastTouchPosition.y - options.touch.tapPrecision &&
          startTouchPosition.y <=
            lastTouchPosition.y + options.touch.tapPrecision
        ) {
    ...
    function updateAxesMinMax() {
      ...
      if (relativeOffset.x !== 0 || relativeOffset.y !== 0) {
        $.each(plot.getAxes(), function (index, axis) {
          if (
            axis.direction === options.touch.pan.toLowerCase() ||
            options.touch.pan.toLowerCase() == "xy"
          ) {
            var min = axis.c2p(
              axis.p2c(axis.touch.min) - relativeOffset[axis.direction]
            );
            var max = axis.c2p(
              axis.p2c(axis.touch.max) - relativeOffset[axis.direction]
            );
      ...
      if (relativeScale !== 1.0) {
        ...
        var range = {
          x: {
            min:
              scaleOriginPixel.x -
              ((scaleOrigin.x / 100) * width) / relativeScale,
            max:
              scaleOriginPixel.x +
              ((1 - scaleOrigin.x / 100) * width) / relativeScale,
          },

          y: {
            min:
              scaleOriginPixel.y -
              ((scaleOrigin.y / 100) * height) / relativeScale,
            max:
              scaleOriginPixel.y +
              ((1 - scaleOrigin.y / 100) * height) / relativeScale,
          },
        };
