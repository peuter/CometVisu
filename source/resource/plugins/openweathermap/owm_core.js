...
(function ($) {
  ...
  var _process = function (e, options) {
    ...
    var forecastURL =
      options.baseURL + "forecast/?" + paramsDefault.join("&") + "&cnt=8";
    var forecastDailyURL =
      options.baseURL + "forecast/?" + paramsDefault.join("&") + "&cnt=39";
    ...
    // Load location data.
    _request(
      options.baseURL + "weather?" + paramsDefault.join("&"),
      function (data) {
        if (data.cod == 200) {
          ...
          if (options.description === "") {
            $("div.openweathermap_value").html(
              "<p>" + options.cityname + "</p>"
            );
  ...
  var _processDataDaily = function (e, url, options) {
    ...
    _request(url, function (data) {
      if (data.cod == 200) {
        ...
        //create legend
        var output =
          '<li><div class="weather-forecast weather-thermo clearfix">';
  ...
  function _proccessSunrise(options) {
    ...
    var output =
      '<li class="sunrise-sunset"><div class="weather-forecast weather-sunrise clearfix" style="float: left;">';
    output += '<div class="weather-icon"></div>';
    output +=
      '<div class="sunrise-sunset">' + d.strftime("%H:%M") + "</div> </div>";
    //time sunset
    d = new Date(options.sunset * 1000);
    output +=
      '<div class="weather-forecast weather-sunset clearfix" style="float: left;">';
    output += '<div class="weather-icon"></div>';
    output +=
   ...
   */
  jOWM.theme.prototype.weatherTemperature = function (
    temperature,
    precision,
    suffix
  ) {
  ...
  jOWM.theme.prototype.weatherDetailItem = function (data, options) {
    ...
    d.locale = options.lang;
    var output =
      '<div class="weather-detailed weather-' + weather.id + ' clearfix">';
    output += ' <div class="weather">';
    output +=
      '  <span class="weather-icon" data-weather-text="' +
      weather.description +
      '" data-weather-code="' +
      weather.id +
      '"></span>';
    ...
    output +=
      ...
      jOWM.theme("weatherTemperature", temperature.temp, 1) +
      "</div>";
    output += "</div>";
    return output;
  };
  ...
  jOWM.theme.prototype.weatherForecastItem = function (data, options) {
    ...
    d.locale = options.lang;
    var output =
      '<div class="weather-forecast weather-' + weather.id + ' clearfix">';
    output += ' <div class="day">' + d.strftime("%H:%M") + "</div>";
    output +=
      ' <div class="weather-icon" data-weather-text="' +
      weather.description +
      '" data-weather-code="' +
      weather.id +
      '"></div>';
    output +=
      ...
      jOWM.theme("weatherTemperature", temperature.temp_max, 0, " °C") +
      "</div>";
    output += "</div>";
    return output;
  };
  ...
  jOWM.theme.prototype.weatherForecastDailyItem = function (data, options) {
    ...
    output +=
      ' <div class="temperature high">' +
      jOWM.theme("weatherTemperature", data.max_temp, 0, " °C") +
      "</div>";
    output +=
      ' <div class="temperature low">' +
      jOWM.theme("weatherTemperature", data.min_temp, 0, " °C") +
      "</div>";
    output += "</div>";
    return output;
  };
