(function (j, q, h) {
  ...
  D = (function () {
    ...
    return {
      ...
      handle_ident(G) {
        ...
        return (
          H(G.language.namedIdentRules.custom) ||
          H(G.language.namedIdentRules.follows, function (I) {
    ...
    return function (M) {
      ...
      for (J = 0; J < G.length; J++) {
        ...
        if (N === h) {
          ...
          if (
            N.name === K.token &&
            (K.values === h || i(K.values, N.value, F))
          ) {
            continue;
          } else {
            if (K.optional !== h && K.optional) {
    ...
    for (K = 0; K < M.length; K++) {
      ...
      if (G === N || M[K].regex.test(N)) {
        return H.createToken(
          I,
          H.reader.current() + H.reader[F ? "peek" : "read"](G.length - 1),
          O,
          J
        );
      }
  ...
  d.prototype = (function () {
    var J = (function () {
      function W(Z) {
        return (
          ...
          Z.language.identFirstLetter.test(Z.reader.current())
        );
      }
      function S(Z) {
      ...
      function P(Z) {
        ...
        for (ab in Z.language.scopes) {
          ...
          for (ae = 0; ae < aa.length; ae++) {
            ...
            if (
              ag !== ah &&
              (!Z.language.caseInsensitive ||
                ah.toUpperCase() !== ag.toUpperCase())
            ) {
              continue;
            }
            (ai = Z.reader.getLine()), (ac = Z.reader.getColumn());
      ...
      return function (Z) {
        ...
        return (
          U(Z) || X(Z) || S(Z) || P(Z) || Q(Z) || T(Z) || V(Z) || R(Z) || Y(Z)
        );
      };
    })();
    function H(S, T) {
    ...
    function N(T, W, U, Q) {
      var V = [],
        ...
        P,
        R;
      B("beforeTokenize", this, { code: T, language: W });
      S = {
        reader: u(T),
        language: W,
      ...
      B("afterTokenize", this, { code: T, parserContext: S });
      return S;
    }
    function F(T, R, Q) {
      var P = [],
        S = (function () {
          ...
          if (Q.showWhitespace) {
            ...
            V =
              new Array(Q.tabWidth).join(String.fromCharCode(8212)) +
    ...
    function K(Q, P, R) {
      ...
      B("beforeHighlight", this, { code: Q, language: T, previousContext: R });
      S = F(N.call(this, Q, T, R, this.options), R, this.options);
      I.call(this, S, R.index ? R.index + 1 : 0);
      B("afterHighlight", this, { analyzerContext: S });
      return S;
    }
    function M(Q) {
    ...
    function I(X, Y) {
      var P, W, Q, U, R, S, T, V;
      B("beforeAnalyze", this, { analyzerContext: X });
    ...
    return {
      ...
      highlightNode(S) {
        ...
        if (
          this.isAlreadyHighlighted(S) ||
          (V = this.matchSunlightNode(S)) === null
        ) {
          return;
        }
        R = V[1];
        Y = 0;
        B("beforeHighlightNode", this, { node: S });
  ...
  j.Sunlight = {
    ...
    registerLanguage(F, J) {
      ...
      for (I in J.customTokens) {
        J.customTokens[I] = v(
          J.customTokens[I].values,
          J.customTokens[I].boundary,
          J.caseInsensitive
        );
      }
      H = [];
    ...
    util: {
      ...
      getNextNonWsToken(G, F) {
        return z(G, F, 1, function (H) {
          return H.name === "default";
        });
      },
      getPreviousNonWsToken(G, F) {
        return z(G, F, -1, function (H) {
          return H.name === "default";
        });
      },
  ...
  j.Sunlight.registerLanguage(t, { punctuation: /(?!x)x/, numberParser: E });
})(this, document);
(function (b, a, d) {
  if (b === d) {
  ...
  b.bind("afterHighlightNode", function (g) {
    ...
    if (
      ...
      b.util.getComputedStyle(g.node, "display") !== "block"
    ) {
      return;
    }
    ...
    for (
      h = this.options.lineNumberStart;
      h <= this.options.lineNumberStart + o;
      h++
    ) {
      l = a.createElement("a");
      f =
        (g.node.id ? g.node.id : this.options.classPrefix + g.count) +
        "-line-" +
        h;
