...
describe("test the notification router", function () {
  ...
  it("should evaluate a message condition", function () {
    ...
    expect(
      cv.core.notifications.Router.evaluateCondition({ condition: true })
    ).toBeTruthy();
    expect(
      cv.core.notifications.Router.evaluateCondition({ condition: false })
    ).toBeFalsy();

    expect(
      cv.core.notifications.Router.evaluateCondition({
        condition() {
          return true;
        },
      })
  ...
  it("should test the target mapping", function () {
    expect(cv.core.notifications.Router.getTarget("popup")).toEqual(
      cv.ui.PopupHandler
    );
    expect(
      cv.core.notifications.Router.getTarget("notificationCenter")
    ).toEqual(cv.ui.NotificationCenter.getInstance());
    ...
    expect(cv.core.notifications.Router.getTarget("speech")).toEqual(
      cv.core.notifications.SpeechHandler.getInstance()
    );
