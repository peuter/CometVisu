...
describe("testing a TR-064 plugin", function () {
  beforeAll(function (done) {
    ...
    qx.io.PartLoader.require(
      ["plugin-tr064"],
      function () {
        cv.util.ScriptLoader.getInstance().addListenerOnce("finished", () => {
          done();
        });
        qx.event.Timer.once(
          function () {
  ...
  it("should test the TR-064:calllist column selector and table filling", function (done) {
    var widgetInstance = this.createTestElement(
      "calllist",
      {
        id: "test",
        device: "testdevice",
        columns: "type;tam;name;caller;date;nameOrCaller",
      },
      "<label>Test</label>"
    );
  ...
  it("should test the TR-064:calllist refresh", function (done) {
    var widgetInstance = this.createTestElement(
      "calllist",
      {
        ...
        device: "testdevice",
        refresh: 1,
        columns: "type;tam;date;nameOrCaller",
      },
      "<label>Test</label>"
    );
    ...
    widgetInstance.addListener("tr064ListRefreshed", function () {
      ...
      expect(widgetInstance.refreshCalllist).toHaveBeenCalledWith(
        "getCallListURI"
      );
      done();
    });
