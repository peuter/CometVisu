...
describe("checking openhab transforms", function () {
  it("should transform switch values", function () {
    ...
    expect(
      cv.Transform.decode({ transform: "OH:switch" }, "Uninitialized")
    ).toEqual(0);
  });

  it("should transform contact values", function () {
    expect(cv.Transform.encode({ transform: "OH:contact" }, 0)).toEqual(
      "CLOSED"
    );
    ...
    expect(cv.Transform.decode({ transform: "OH:contact" }, "CLOSED")).toEqual(
      0
    );

    expect(cv.Transform.decode({ transform: "OH:contact" }, "NaN")).toEqual(0);
    expect(
      cv.Transform.decode({ transform: "OH:contact" }, "Uninitialized")
    ).toEqual(0);
  });

  it("should transform rollershutter values", function () {
    expect(cv.Transform.encode({ transform: "OH:rollershutter" }, 0)).toEqual(
      "UP"
    );
    expect(cv.Transform.encode({ transform: "OH:rollershutter" }, 1)).toEqual(
      "DOWN"
    );
    expect(
      cv.Transform.encode({ transform: "OH:rollershutter" }, "UP")
    ).toEqual("UP");
    expect(
      cv.Transform.decode({ transform: "OH:rollershutter" }, "DOWN")
    ).toEqual(1);
    expect(
      cv.Transform.decode({ transform: "OH:rollershutter" }, "UP")
    ).toEqual(0);
    expect(cv.Transform.decode({ transform: "OH:rollershutter" }, 0)).toEqual(
      0
    );
    expect(
      cv.Transform.decode({ transform: "OH:rollershutter" }, "Uninitialized")
    ).toEqual(0);
    expect(
      cv.Transform.decode({ transform: "OH:rollershutter" }, "NaN")
    ).toEqual(0);
  });
  ...
  it("should transform dimmer values", function () {
    ...
    expect(
      cv.Transform.decode({ transform: "OH:dimmer" }, "Uninitialized")
    ).toEqual(0);
  });
  ...
  it("should transform number values", function () {
    ...
    expect(
      cv.Transform.decode({ transform: "OH:number" }, "Uninitialized")
    ).toEqual(0);
  });

  it("should transform string values", function () {
    expect(
      cv.Transform.encode({ transform: "OH:string" }, "example string")
    ).toEqual("example string");
    expect(
      cv.Transform.decode({ transform: "OH:string" }, "example string")
    ).toEqual("example string");
  });
  ...
  it("should transform datetime values", function () {
    ...
    expect(cv.Transform.encode({ transform: "OH:datetime" }, date)).toEqual(
      date.toLocaleDateString()
    );
    expect(
      cv.Transform.encode({ transform: "OH:datetime" }, dateString)
    ).toEqual(dateString);
    expect(
      cv.Transform.decode({ transform: "OH:datetime" }, dateString)
    ).toEqual(date);
    ...
    expect(
      cv.Transform.decode({ transform: "OH:datetime" }, "Uninitialized")
    ).toEqual("-");
  });
  ...
  it("should transform time values", function () {
    ...
    expect(cv.Transform.encode({ transform: "OH:time" }, date)).toEqual(
      date.toLocaleTimeString()
    );
    expect(
      cv.Transform.encode({ transform: "OH:time" }, date.toLocaleTimeString())
    ).toEqual(date.toLocaleTimeString());
    date = new Date();
    date.setHours(12);
    date.setMinutes(53);
    date.setSeconds(13);
    ...
    ).toEqual(date.toLocaleTimeString("de-DE"));

    expect(cv.Transform.decode({ transform: "OH:time" }, "NaN")).toEqual("-");
    expect(
      cv.Transform.decode({ transform: "OH:time" }, "Uninitialized")
    ).toEqual("-");
  });
  ...
  it("should transform color values", function () {
    ...
    // decode HSV -> RGB
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "228, 7.8, 25.1")
    ).toEqual(
      new Map([
        ["r", 59],
        ["g", 60],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "87.7, 24.1, 21.2")
    ).toEqual(
      new Map([
        ["r", 47],
        ["g", 54],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "36.5, 20.9, 43.1")
    ).toEqual(
      new Map([
        ["r", 109],
        ["g", 100],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "36.5, 20.9, 0")
    ).toEqual(
      new Map([
        ["r", 0],
        ["g", 0],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "120, 60, 10")
    ).toEqual(
      new Map([
        ["r", 10],
        ["g", 25],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "200, 60, 10")
    ).toEqual(
      new Map([
        ["r", 10],
        ["g", 20],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "260, 60, 10")
    ).toEqual(
      new Map([
        ["r", 14],
        ["g", 10],
    ...
    expect(
      cv.Transform.decode({ transform: "OH:color" }, "320, 60, 10")
    ).toEqual(
      new Map([
        ["r", 25],
        ["g", 10],
    ...
    expect(cv.Transform.decode({ transform: "OH:color" }, "0,0,0")).toEqual(
      new Map([
        ["r", 0],
        ["g", 0],
        ["b", 0],
