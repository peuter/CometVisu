...
describe("test the NotificationCenter", function () {
  ...
  it("should toggle the visibility", function (done) {
    ...
    center.show();
    setTimeout(function () {
      expect(window.getComputedStyle(element)["transform"]).toEqual(
  ...
  it("should perform a message action", function () {
    ...
    cv.ui.NotificationCenter.performAction(
      center.getMessages().getLength() - 1
    );
  ...
  it("should test the interaction handling with list items", function () {
    ...
    if (window.PointerEvent) {
      ...
      expect(center.deleteMessage).toHaveBeenCalledWith(
        0,
        jasmine.any(qx.event.type.Event)
      );
    }
