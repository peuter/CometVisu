...
describe("testing a designtoggle widget", function () {
  ...
  it("should trigger the designtoggle action", function () {
    ...
    expect(cv.util.Location.setHref).toHaveBeenCalledWith(
      loc + "?design=metal" + anchor
    );

    spyOn(cv.util.Location, "getHref").and.returnValue(
      loc + "?design=pure" + anchor
    );
    Reg.fireEvent(actor, "tap", qx.event.type.Event, []);
    ...
    expect(cv.util.Location.setHref).toHaveBeenCalledWith(
      loc + "?other=parameter&design=metal" + anchor
    );
  });
});
