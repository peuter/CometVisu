...
describe("testing a image widget", function () {
  ...
  it("should test the image creator", function () {
    ...
    expect(element.querySelector("img").getAttribute("src")).toBe(
      "/source/resource/icons/comet_64_ff8000.png"
    );
    expect(element.querySelector("img").getAttribute("style")).toBe(
      "width:100%;"
    );
  });
  ...
  it("should test the image creator and refreshing", function () {
    ...
    expect(widget.querySelector("img").getAttribute("style")).toBe(
      "width:50%;height:51%;"
    );
  });
...
describe("testing the refresh caching of the image widget", function () {
  it("should test refreshing with full cache control", function (done) {
    ...
    expect(domElement.querySelector("img").getAttribute("src")).toBe(
      "/source/resource/icons/comet_64_ff8000.png"
    );

    qx.event.Timer.once(
      function () {
        expect(domElement.querySelector("img").getAttribute("src")).toMatch(
          /^\/source\/resource\/icons\/comet_64_ff8000.png\?/
        );
