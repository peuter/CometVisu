...
describe("testing a infotrigger widget", function () {
  var simulateEvent = function (actor, type) {
    ...
    qx.event.Registration.fireEvent(actor, type, qx.event.type.Pointer, [
      nativeEvent,
      actor,
      actor,
      true,
  ...
  it("should test the infotrigger creator", function () {
    ...
    var actors = Array.from(widget.getElementsByTagName("*")).filter(function (
      m
    ) {
  ...
  it("should test the infotrigger creator centered", function () {
    var obj = this.createTestElement("infotrigger", {
      align: "right",
      infoposition: "middle",
    });
    var widget = obj.getWidgetElement();
  ...
  it("should trigger the infotrigger action", function () {
    ...
    expect(creator.sendToBackend).toHaveBeenCalledWith(
      2,
