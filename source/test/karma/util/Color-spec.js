...
describe("checking color class", function () {
  it("should create Color with default values", function () {
    ...
    c.changeComponent("RGB-b", 0);
    expect(c.getComponent("xy").x).toBeCloseTo(0.3, 4);
    expect(c.getComponent("xy").y).toBeCloseTo(0.6, 4);
    c.changeComponent("RGB-r", 0);
    c.changeComponent("RGB-g", 0);
    ...
    expect(c.getComponent("xy").y).toBeCloseTo(0.329, 4); // sRGB white point D65
  });

  it("should create Color with non default values but no white", function () {
    let c = new cv.util.Color(
      { x: 0.65, y: 0.34, Y: 0.2126 },
      { x: 0.31, y: 0.61, Y: 0.7152 },
      { x: 0.16, y: 0.07, Y: 0.0722 }
    );
    expect(c.getComponent("hsv")).toEqual({ h: 0, s: 0, v: 0 });
    c.changeComponent("RGB-r", 1);
    c.changeComponent("RGB-g", 0);
  ...
  it("should work with RGB colors", function () {
    ...
    c.changeComponent("RGB-g", 1);
    expect(c.getComponent("xy").x).toBeCloseTo(0.41932, 4);
  ...
  it("should work with RGBW colors", function () {
    ...
    let c = new cv.util.Color(
      { x: 0.64, y: 0.33, Y: 0.2126 },
      { x: 0.3, y: 0.6, Y: 0.7152 },
      { x: 0.15, y: 0.06, Y: 0.0722 },
      { x: 0.31, y: 0.32, Y: 1 }
    );
    c.changeComponent("RGBW-r", 0);
    c.changeComponent("RGBW-g", 0);
  ...
  it("should work with HSV colors", function () {
    ...
    c.changeComponent("xy", c.getComponent("xy")); // force validation of color
    expect(
      c.getComponent("h") - (c.getComponent("h") > 0.5 ? 1 : 0)
    ).toBeCloseTo(0, 4); // wrap to 0
    expect(c.getComponent("s")).toBeCloseTo(1, 4);
    expect(c.getComponent("v")).toBeCloseTo(1, 4);
    expect(c.getComponent("RGB-r")).toBeCloseTo(1, 4);
    expect(c.getComponent("RGB-g")).toBeCloseTo(0, 4);
    ...
    expect(c.getComponent("xy").y).toBeCloseTo(0.32876, 4);
    c.changeComponent("xy", c.getComponent("xy")); // force validation of color
    expect(c.getComponent("h")).toBeCloseTo(0.5, 4);
    expect(c.getComponent("s")).toBeCloseTo(1, 4);
    expect(c.getComponent("v")).toBeCloseTo(1, 4);
  ...
  it("should work with color temperature", function () {
    ...
    expect(c.getComponent("xy").x).toBeCloseTo(0.25252, 3); // => limit at coordinates of 25000 K
    expect(c.getComponent("xy").y).toBeCloseTo(0.252218, 3);
    c.changeComponent("xy", c.getComponent("xy")); // force validation of color
    expect(c.getComponent("T")).toBeCloseTo(12500, 4); // colors can only be converted to temperatures in the range 2000...12500

    c.changeComponent("RGB-r", 1.0);
    ...
    const D65_xyY = {
      x: D65_XYZ.X / (D65_XYZ.X + D65_XYZ.Y + D65_XYZ.Z),
      y: D65_XYZ.Y / (D65_XYZ.X + D65_XYZ.Y + D65_XYZ.Z),
      Y: 1,
    };
    // AdobeRGB 1998 values, to test a not standard color, and still use published values as a reference
    let c2 = new cv.util.Color(
      { x: 0.64, y: 0.33, Y: 0.297361 },
      { x: 0.21, y: 0.71, Y: 0.627355 },
      { x: 0.15, y: 0.06, Y: 0.075285 },
      D65_xyY
    );
  ...
  it("should work with LCh colors", function () {
    ...
    expect(c.getComponent("RGB-g")).toBeCloseTo(0.95546 / 1.369449, 3); // linear RGB
  ...
  it("should convert a wavelength to xy coordiantes", function () {
    ...
    expect(answer.x).toBeCloseTo(0.6955, 5);
    expect(answer.y).toBeCloseTo(0.3045, 5);
  });
  ...
  it("should convert xy to sRGB", function () {
    ...
    expect(answer.b).toBeCloseTo(0.80049, 3);
  });
  ...
  it("should convert according to a inverted curve", function () {
    ...
    let curve = [0, 0.2, 0.4, 0.7, 1];
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.0, curve, 100), curve, 100)
    ).toBeCloseTo(0.0, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.1, curve, 100), curve, 100)
    ).toBeCloseTo(0.1, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.2, curve, 100), curve, 100)
    ).toBeCloseTo(0.2, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.3, curve, 100), curve, 100)
    ).toBeCloseTo(0.3, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.4, curve, 100), curve, 100)
    ).toBeCloseTo(0.4, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.5, curve, 100), curve, 100)
    ).toBeCloseTo(0.5, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.6, curve, 100), curve, 100)
    ).toBeCloseTo(0.6, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.7, curve, 100), curve, 100)
    ).toBeCloseTo(0.7, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.8, curve, 100), curve, 100)
    ).toBeCloseTo(0.8, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(0.9, curve, 100), curve, 100)
    ).toBeCloseTo(0.9, 4);
    expect(
      cv.util.Color.invCurve(cv.util.Color.curve(1.0, curve, 100), curve, 100)
    ).toBeCloseTo(1.0, 4);
  });
