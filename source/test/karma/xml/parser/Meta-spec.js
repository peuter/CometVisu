...
describe("testing the meta parser", function () {
  it("should test the meta parser", function () {
    ...
    parser.parse(xml, function () {
      ...
      ["Motion_FF_Dining", "Motion_FF_Corridor", "Motion_FF_Kitchen"].forEach(
        function (address) {
      ...
      // template
      expect(
        cv.parser.WidgetParser.getTemplates().hasOwnProperty("test")
      ).toBeTruthy();
      expect(cv.parser.WidgetParser.getTemplates().test.trim()).toEqual(
        "<root><text><label>Test template</label></text></root>"
      );
