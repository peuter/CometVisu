...
class CometVisuEditorMockup extends BasePage {
  constructor(target) {
    ...
    this.url =
      "http://localhost:8000/" +
      this.target +
      "/index.html?testMode=true&manager=1";
    this.pageLoaded = this.and(this.isVisible($("#manager")), this.mockupReady);
  }
    ...
    request(
      {
        ...
        uri:
          ...
          encodeURIComponent(
            "/" +
              this.target +
              "/rest/manager/index.php/fs?path=visu_config_mockup.xml"
          ),
        body: config,
      },
  ...
  enableExpertMode() {
    return browser.executeAsyncScript(function (callback) {
      const editor =
        cv.ui.manager.control.ActionDispatcher.getInstance().getFocusedWidget();
  ...
  openWidgetElement(selector, edit) {
    return browser.executeAsyncScript(
      function (selector, edit, callback) {
        /**
         * @param editor
         * @param sel
         * @param edit
