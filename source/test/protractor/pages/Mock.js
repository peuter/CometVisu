...
class CometVisuMockup extends BasePage {
  constructor(target) {
    ...
    this.url =
      "http://localhost:8000/" +
      this.target +
      "/index.html?config=mockup&testMode=true&enableCache=false";
    this.mockupReady = false;

    this.pageLoaded = this.and(this.isVisible($("#id_")), this.mockupReady);
  }
    ...
    request(
      {
        ...
        uri:
          ...
          encodeURIComponent(
            "/" + this.target + "/resource/config/visu_config_mockup.xml"
          ),
        body: config,
      },
