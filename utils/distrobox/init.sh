...
if [ ! -f /root/.first_run_complete ]; then
    ...
    NO_ENTRYPOINT_APACHE=true sudo -E /usr/local/bin/cometvisu-entrypoint > /root/init.log 2>&1
    ...
    # create symlinks for distrobox-host-exec to allow running docker and kde-open
